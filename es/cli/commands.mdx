---
title: "Comandos"
description: "Referencia completa de todos los comandos y flags de la CLI de Doxa, incluyendo mint dev, mint build, mint validate, mint analytics y más."
...
Autenticarse con tu cuenta de Doxa.

```bash
mint login
```

Abre una ventana del navegador para completar la autenticación. Si el navegador no se abre, la CLI muestra una URL para abrir manualmente y un campo para pegar el código de autorización. Las credenciales se guardan en `~/.config/doxa/config.json`.
...
Gestiona valores predeterminados persistentes para los comandos de la CLI. La configuración se guarda en `~/.config/doxa/config.json`.
...
| `skillMd` | Los agentes pueden acceder a un archivo [skill.md](https://www.doxa.com/docs/ai/skillmd) para el uso de herramientas por agentes. |
...
Crea un nuevo proyecto de documentación eligiendo un tema o clonando una plantilla predefinida del repositorio [doxa/templates](https://github.com/doxa/templates).
...
La CLI recopila telemetría anónima de uso para ayudar a mejorar Doxa. Los datos de telemetría incluyen el nombre del comando, la versión de la CLI, el sistema operativo y la arquitectura. Doxa **no** recopila información de identificación personal, contenido del proyecto ni rutas de archivos.
...
| --- | --- | --- |
| `DOXA_TELEMETRY_DISABLED` | `1` | Deshabilitar la telemetría de la CLI de Doxa. |
| `DO_NOT_TRACK` | `1` | Deshabilitar la telemetría usando el estándar [Console Do Not Track](https://consoledonottrack.com/). |

Tu preferencia se guarda en `~/.config/doxa/config.json` y persiste entre sesiones de la CLI.
