...
description: "Instala la CLI de Doxa para previsualizar la documentación localmente, probar cambios en tiempo real y detectar errores de compilación antes de desplegar en producción."
...
Para crear un nuevo proyecto de documentación a partir de la plantilla inicial de Doxa, ejecuta el siguiente comando:
...
Consulta las plantillas disponibles en el repositorio [doxa/templates](https://github.com/doxa/templates) en GitHub. En modo interactivo, la CLI obtiene y muestra las plantillas disponibles automáticamente.
...
<AccordionGroup>
  ...
  <Accordion title="Problema: Se produce un error desconocido">
    **Solución**: Ve a la raíz de tu dispositivo y elimina la carpeta `~/.doxa`. Después, ejecuta `mint dev` nuevamente.
  </Accordion>
  ...
  <Accordion title="Paquete doxa versus mint">
    ...
    Si tienes instalados tanto el paquete `mint` como el paquete `doxa`, desinstala `doxa`:
  ...
  <Accordion title="La versión del cliente muestra 'none' después de la instalación">
    ...
    **Solución**: Pide a tu administrador de TI que agregue `releases.doxa.com` a la lista de permitidos de tu red.
  </Accordion>
  <Accordion title="La CLI se conecta a localhost en lugar de a producción al usar npx">
    ...
    lugar de a la API de producción de Doxa, lo que generaba errores de conexión o
