...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="proxy" type="boolean" defaultOpen="True">
      Indica si las solicitudes de API deben pasar por el servidor proxy de Doxa. El valor predeterminado es `true`.

      Cuando es `true`, las solicitudes del playground se enrutan a través de los servidores de Doxa. Cuando es `false`, el playground envía las solicitudes directamente desde el navegador a tu API. Establécelo en `false` cuando tu API acepte solicitudes directas del navegador y no necesites que Doxa actúe como proxy del tráfico. Por ejemplo, cuando tu API requiere encabezados específicos que no se pueden reenviar a través del proxy o cuando necesitas que la solicitud se origine directamente desde el navegador del usuario por motivos de autenticación.
    </ResponseField>
