...
Doxa aloja un archivo `skill.md` en la raíz de tu proyecto que describe lo que los agentes de IA pueden hacer con tu producto.
...
Doxa genera automáticamente un archivo `skill.md` para tu proyecto analizando tu documentación con un bucle de agentes. Este archivo se mantiene actualizado a medida que realizas cambios en tu documentación y no requiere mantenimiento. Opcionalmente, puedes añadir un archivo `skill.md` personalizado en la raíz de tu proyecto que reemplace al generado automáticamente.

<Note>Generar o actualizar un archivo `skill.md` puede tardar hasta 24 horas.</Note>

Consulta tu `skill.md` añadiendo `/skill.md` a la URL de tu sitio de documentación. Doxa solo genera archivos `skill.md` para sitios de documentación que sean públicos.
...
<Note>
  Si utilizas un [proxy inverso](/es/deploy/reverse-proxy), configúralo para reenviar las rutas `/skill.md`, `/.well-known/skills/*` y `/.well-known/agent-skills/*` a tu subdominio de Doxa.
</Note>
...
Doxa genera un archivo `skill.md` siguiendo la [especificación de agentskills.io](https://agentskills.io/specification). El archivo generado incluye:
...
Añade archivos de skill personalizados para reemplazar el `skill.md` generado automáticamente. Doxa admite alojar un único archivo de skill y un directorio para múltiples skills. Si eliminas todos los archivos de skill personalizados, Doxa genera un nuevo archivo `skill.md`.
...
Añade múltiples archivos de skill al directorio `.doxa/skills/` de tu proyecto. Cada skill debe estar en su propio subdirectorio con un archivo `SKILL.md`:
...
<Note>
  Puedes usar ambos enfoques juntos con un archivo `skill.md` en la raíz y un directorio `.doxa/skills/`. El índice de descubrimiento incluye todas las skills.
</Note>
...
Si tus archivos de skill están en otra ubicación de tu repositorio (por ejemplo, en un directorio `plugins/` o `skills/`), puedes crear un enlace simbólico de `.doxa/skills` hacia esa ubicación en lugar de duplicar archivos:
...
Doxa resuelve los enlaces simbólicos durante el despliegue, por lo que los archivos de skill se descubren y se sirven como si estuvieran directamente dentro de `.doxa/skills/`. Esto funciona tanto con enlaces simbólicos de directorios como de skills individuales.
...
---
name: doxa
description: Construye y mantén sitios de documentación con Doxa. Úsalo al crear páginas de documentación, configurar la navegación, agregar componentes o configurar referencias de API.
license: MIT
...
Doxa aloja directorios de skills en `/.well-known/skills/` y `/.well-known/agent-skills/` que los agentes pueden usar para descubrir y obtener tus archivos de skills de forma programática.
...
Las URL de la tarjeta (`url`, `documentationUrl`, `provider.url` y cada URL de skill) se construyen a partir del dominio personalizado configurado cuando está disponible, de modo que la tarjeta publicada siempre anuncia el dominio canónico en lugar del subdominio `*.doxa.app`.

<Note>
  Si usas un [reverse proxy](/es/deploy/reverse-proxy), configúralo para reenviar `/.well-known/agent-card.json` a tu subdominio de Doxa.
</Note>
