...
description: "Configura la autenticación de usuarios para tu sitio con OAuth, JWT o enlaces compartidos para controlar el acceso a páginas y referencias de API."
keywords: ['authentication', 'auth', 'OAuth', 'JWT', 'password']
---

<Info>
  La autenticación del dashboard de Doxa está disponible en todos los planes.

  La autenticación por contraseña, OAuth y JWT requiere un [plan Enterprise](https://doxa.com/pricing?ref=authentication).
</Info>

<Warning>
  La autenticación solo está disponible para sitios alojados en un dominio personalizado o subdominio de Doxa (por ejemplo, `docs.ejemplo.com` o `ejemplo.doxa.dev`). La autenticación **no es compatible** cuando se usa una [ruta base personalizada](/es/deploy/docs-subpath) (por ejemplo, `ejemplo.com/docs`).
</Warning>
...
<Tabs>
    ...
    * Tus requisitos de seguridad permiten compartir contraseñas entre usuarios.
    ...
    <Steps>
      <Step title="Crea una contraseña.">
        1. En tu dashboard, ve a [Authentication](https://dashboard.doxa.com/products/authentication).
  ...
  <Tab title="Dashboard de Doxa">
    ### Requisitos previos del Dashboard de Doxa

    * Todas las personas que necesiten acceder a tu documentación deben ser miembros de tu organización de Doxa.

    ### Configuración del Dashboard de Doxa

    <Steps>
      <Step title="Habilita la autenticación del Dashboard de Doxa.">
        1. En tu dashboard, ve a [Authentication](https://dashboard.doxa.com/products/authentication).
        2. Habilita la autenticación.
        3. En la sección **Custom Authentication**, haz clic en **Doxa Auth**.
        4. Haz clic en **Enable Doxa Auth**.

        Después de habilitar la autenticación de Doxa, tu sitio se vuelve a implementar automáticamente. Una vez que finalice la implementación, cualquier persona que visite tu sitio deberá iniciar sesión en tu organización de Doxa para acceder a tu contenido.
      </Step>

      <Step title="Agrega usuarios autorizados.">
        1. En tu dashboard, ve a [Members](https://dashboard.doxa.com/settings/organization/members).
    ...
    **Habilita la autenticación de Doxa** en la configuración de tu dashboard.
    ...
    * Capacidad para crear un endpoint de API accesible mediante tokens de acceso OAuth (opcional, para habilitar el control de acceso basado en grupos).
    ...
    <Steps>
      <Step title="Configura tus ajustes de OAuth.">
        ...
        * **Info API URL** (opcional): Endpoint en tu servidor al que Doxa llama para obtener información del usuario. Obligatorio para el control de acceso basado en grupos. Si se omite, el flujo de OAuth solo verifica la identidad.
        * **Logout URL** (opcional): La URL de cierre de sesión nativa de tu proveedor de OAuth. Cuando los usuarios cierran sesión, Doxa valida la redirección de cierre de sesión frente a esta URL configurada por motivos de seguridad. La redirección solo se completa si coincide exactamente con el `logoutUrl` configurado. Si no configuras una Logout URL, los usuarios se redirigen a `/login`. Doxa redirige a los usuarios con una solicitud `GET` y no agrega parámetros de consulta, por lo que debes incluir cualquier parámetro (por ejemplo, `returnTo`) directamente en la URL.
      ...
      <Step title="Configura tu servidor OAuth.">
        1. Copia la **Redirect URL** de tus [ajustes de autenticación](https://dashboard.doxa.com/products/authentication).
      ...
      <Step title="Crea tu endpoint de información de usuario (opcional).">
        ...
        * Devuelva los datos de usuario en el formato `User`. Consulta [Formato de datos de usuario](#user-data-format) para obtener más información.
        ...
        Doxa llama a este endpoint con el token de acceso de OAuth para obtener la información del usuario. No se envían parámetros de consulta adicionales.

        Agrega la URL de este endpoint al campo **Info API URL** en tus [ajustes de autenticación](https://dashboard.doxa.com/products/authentication).
      </Step>
    </Steps>
  ...
  <Tab title="JWT">
    ...
    * Un servicio de backend que pueda crear URL de redirección.
    ...
    <Steps>
      ...
      <Step title="Integra la autenticación de Doxa en tu flujo de inicio de sesión.">
    ...
    * Genera un JWT con los datos del usuario en el formato de Doxa.
