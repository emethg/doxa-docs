---
title: "Cloudflare"
description: "Despliega tu documentación de Doxa en una subruta de tu dominio usando Cloudflare Workers con configuración paso a paso y ajustes de DNS."
...
Asegúrate de que la cabecera `HOST` se reenvíe correctamente en la configuración de tu Worker. Si no se reenvían correctamente las cabeceras, las solicitudes de verificación fallan.

<div id="configure-routing">
  ### Configurar el enrutamiento
</div>

En tu dashboard de Cloudflare, selecciona **Edit Code** y añade el siguiente script en el código de tu Worker. Consulta la [documentación de Cloudflare](https://developers.cloudflare.com/workers-ai/get-started/dashboard/#development) para obtener más información sobre cómo editar un Worker.
...
async function handleRequest(request) {
  try {
    ...
    if (/^\/docs/.test(urlObject.pathname)) {
      // Entonces hacer proxy a Doxa
      const DOCS_URL = "[SUBDOMAIN].doxa.dev";
...
Después de desplegar tu código, prueba tu Worker para asegurarte de que dirige a tu documentación de Doxa.

1. Prueba usando la URL de vista previa del Worker: `your-worker.your-subdomain.workers.dev/docs`
2. Verifica que el Worker dirija a tu documentación de Doxa y a tu sitio web.
...
Si usas Webflow para alojar tu sitio principal y quieres servir la documentación de Doxa en `/docs` en el mismo dominio, tendrás que configurar un enrutamiento personalizado mediante Cloudflare Workers para redirigir mediante proxy todo el tráfico que no sea de docs hacia tu sitio principal.

<Warning>
  Asegúrate de que tu sitio principal esté configurado en una landing page antes de desplegar este Worker, o los visitantes de tu sitio principal podrían ver errores.
</Warning>
