...
description: "Instala y configura la aplicación GitHub de Doxa en tu instancia de GitHub Enterprise Server para despliegues automáticos de documentación y sincronización."
keywords: ["GitHub Enterprise Server", "GHES", "GitHub Enterprise"]
---

En esta guía se explica cómo configurar la aplicación de GitHub de Doxa en tu instalación de GitHub Enterprise Server (GHES). Para conectar una instancia de GHES a Doxa, debes crear una versión local de nuestra aplicación dentro de tu entorno autoalojado que se comunique con nuestro servidor remoto.
...
* Los endpoints de la API de Doxa (https://leaves.doxa.com)
...
* Conexiones desde la IP estática de Doxa: `54.242.90.151`
* HTTPS (puerto 443) a los dominios de servicio de Doxa
* Resolución de DNS para los dominios de servicio de Doxa
...
- **Description:** `Integration with Doxa services`
...
<Steps>
  ...
  <Step title="Configurar la URL del webhook">
    ...
    * **Webhook URL:** `https://leaves.doxa.com/github-enterprise/:subdomain` (reemplaza `:subdomain` con la URL que te proporcionemos)
    * **Webhook secret:** Genera una cadena aleatoria (de 32 caracteres o más) y guárdala de forma segura. Doxa también puede generarla y proporcionártela.
  </Step>
</Steps>
...
* Integrar tu entorno GHES con un despliegue existente de Doxa.
* Integrar tu entorno GHES con un nuevo despliegue de Doxa que aprovisionamos para ti.

Una vez que tu entorno GHES esté integrado con un despliegue de Doxa, podrás habilitar los webhooks de tu GitHub App.
...
<Steps>
  ...
  <Step title="Probar el acceso al repositorio">
    ...
    2. Asegúrate de que Doxa responda correctamente.
  </Step>
</Steps>
...
Ponte en contacto con tu representante de customer success con quien ya hayas hablado en Doxa, o con nuestro equipo de soporte en <a href="mailto:support@doxa.com">support@doxa.com</a> con:
