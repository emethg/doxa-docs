...
description: "Configura encabezados de Content Security Policy para permitir recursos de Doxa manteniendo la seguridad en proxies inversos y redes estrictas."
keywords: ["Content Security Policy", "CSP", "directivas", "encabezados de seguridad", "cortafuegos", "prevención de XSS"]
---

Content Security Policy (CSP) es un estándar de seguridad que ayuda a prevenir ataques de cross-site scripting (XSS) controlando qué recursos puede cargar una página web. Doxa proporciona una CSP predeterminada que protege a la mayoría de los sitios. Si alojas tu documentación detrás de un proxy inverso o un cortafuegos que sobrescribe la CSP predeterminada, puede que necesites configurar encabezados CSP para que las funciones se ejecuten correctamente.
...
| `*.doxa.dev` | Contenido de la documentación | `connect-src`, `frame-src` | Obligatorio |
| `*.doxa.com` | Dashboard, API, proxy de Analytics | `connect-src` | Obligatorio |
| `leaves.doxa.com` | API del assistant | `connect-src` | Obligatorio |
...
| `doxa.s3.us-west-1.amazonaws.com` | Imágenes alojadas en S3 | `img-src` | Obligatorio |
...
img-src 'self' data: blob: d3gk2c5xim1je2.cloudfront.net mintcdn.com *.mintcdn.com cdn.jsdelivr.net doxa.s3.us-west-1.amazonaws.com;
connect-src 'self' *.doxa.dev *.doxa.com d1ctpt7j8wusba.cloudfront.net mintcdn.com *.mintcdn.com
www.googletagmanager.com cdn.segment.com plausible.io us.posthog.com browser.sentry-cdn.com hcaptcha.com *.hcaptcha.com;
frame-src 'self' *.doxa.dev hcaptcha.com *.hcaptcha.com;
```
