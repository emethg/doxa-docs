...
description: "Soluciona solicitudes bloqueadas y resuelve conflictos de reglas WAF de Cloudflare al servir documentación de Doxa a través de Cloudflare Workers."
keywords: ["Cloudflare WAF", "firewall rules", "Bot Fight Mode", "403 errors"]
---

Si tu sitio de documentación muestra errores 500 tras unos segundos o la navegación se vuelve lenta, el firewall de Cloudflare podría estar bloqueando solicitudes a los recursos de Doxa.
...
- Errores 403 en la consola del navegador en solicitudes a las rutas `/doxa-assets/*`
...
El Firewall de aplicaciones web (WAF) y el Bot Fight Mode de Cloudflare pueden marcar como sospechosas las solicitudes de recursos de Doxa debido a:
...
Crea una regla de firewall en Cloudflare para excluir los recursos de Doxa de las comprobaciones de seguridad.
...
**Nombre de la regla:** Permitir assets de Doxa
...
- Usar el operador `contains` con `/doxa-assets/*` - El `*` se interpreta como un carácter literal, no como un comodín
- Usar `equals` para la ruta URI - Esto solo coincide con la ruta exacta `/doxa-assets/` y no con subrutas
...
```
Regla: Permitir recursos de Doxa
Estado: Habilitado
