...
sidebarTitle: "AWS"
description: "Despliega tu documentación de Doxa en una subruta en AWS usando Route 53 para el enrutamiento DNS y CloudFront como CDN con funciones Lambda@Edge."
...
3. En Origin domain, ingresa `[SUBDOMAIN].doxa.dev`, donde `[SUBDOMAIN]` es el subdomain único de tu proyecto.

<Frame>
    ![Página «Create distribution» de CloudFront mostrando «acme.doxa.dev» como Origin domain.](/images/cloudfront/origin-name.png)
  </Frame>
...
2. Busca tu URL de staging que refleje el dominio principal. Esto varía según cómo esté alojada tu página de inicio. Por ejemplo, la URL de staging de Doxa es [doxa-landing-page.vercel.app](https://doxa-landing-page.vercel.app).
...
A este punto, deberías tener dos Origins: uno con `[SUBDOMAIN].doxa.app` y otro con tu URL de staging.

<Frame>
  ![Página de CloudFront "Origins" con dos orígenes: uno para doxa y otro para doxa-landing-page.](/images/cloudfront/final-origins.png)
</Frame>
...
- **Si un usuario llega a tu subruta personalizada**, redirigir a `[SUBDOMAIN].doxa.dev`.
...
Crea un comportamiento con un **Path pattern** de la subruta que elijas, por ejemplo `/docs`, con **Origin and origin groups** apuntando a la URL `.doxa.dev` (en nuestro caso `acme.doxa.dev`).
...
<Frame>
  ![Página "Create behavior" de CloudFront con un "Path pattern" de "/docs/*" y "Origin and origin groups" apuntando a la URL `acme.doxa.dev`.](/images/cloudfront/behavior-1.png)
</Frame>
...
Crea un comportamiento con un **Path pattern** que sea la subruta que elijas seguida de `/*`, por ejemplo `/docs/*`, y con **Origin and origin groups** apuntando a la misma URL `.doxa.dev`.
...
1. Cambia **Origin and origin groups** del comportamiento predeterminado a la URL de staging (en nuestro caso, `doxa-landing-page.vercel.app`).
...
Todas las páginas deberían redirigir a tu página de inicio principal, pero si agregas la subruta que elegiste, por ejemplo `/docs`, a la URL, deberías ver que te lleva a tu instancia de documentación de Doxa.
