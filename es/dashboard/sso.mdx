...
<Info>
  SSO está disponible en los [planes Enterprise](https://doxa.com/pricing?ref=sso).
</Info>

Los administradores de planes Enterprise pueden configurar SAML SSO para Okta o Microsoft Entra directamente desde el Dashboard de Doxa. Para otros proveedores como Google Workspace u Okta OIDC, [contáctanos](mailto:support@doxa.com) para configurar SSO.
...
<Steps>
  <Step title="Configura Okta SSO en tu Dashboard de Doxa">
    1. En tu Dashboard de Doxa, navega a la página de [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso).
  ...
  <Step title="Crea una aplicación SAML en Okta">
    ...
    2. Ingresa lo siguiente desde Doxa:
        * **Single sign on URL**: la URL que copiaste desde tu Dashboard de Doxa
        * **Audience URI**: la URI que copiaste desde tu Dashboard de Doxa
  ...
  <Step title="Guarda en Doxa">
    De vuelta en el Dashboard de Doxa, pega la URL de metadata y haz clic en **Guardar cambios**.
  </Step>
</Steps>
...
<Steps>
  <Step title="Configurar Microsoft Entra SSO en el Dashboard de Doxa">
    1. En el Dashboard de Doxa, navega a la página de [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso).
  ...
  <Step title="Configurar SAML en Microsoft Entra">
    ...
    3. En **Basic SAML Configuration**, introduce lo siguiente:
        * **Identifier (Entity ID)**: la Audience URI de Doxa
        * **Reply URL (Assertion Consumer Service URL)**: la Single sign on URL de Doxa
  ...
  <Step title="Guardar en Doxa">
    De vuelta en el Dashboard de Doxa, pega la URL de metadatos y haz clic en **Guardar cambios**.
  </Step>
  <Step title="Asignar usuarios">
    En Microsoft Entra, navega a **Users and groups**. Asigna los usuarios que deberían tener acceso a tu Dashboard de Doxa.
  </Step>
</Steps>
...
Cuando habilites el aprovisionamiento JIT (just-in-time), los usuarios que inicien sesión a través de tu proveedor de identidad se agregarán automáticamente a tu organización de Doxa.

<Note>
  El aprovisionamiento JIT solo funciona para inicios de sesión iniciados por el IdP. Los usuarios deben iniciar sesión desde tu proveedor de identidad (dashboard de Okta o portal de Microsoft Entra) en lugar de hacerlo desde la página de inicio de sesión de Doxa.
</Note>

Para habilitar el aprovisionamiento JIT, debes tener SSO habilitado. Ve a la página de [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso) en tu dashboard, configura el SSO y luego habilita el aprovisionamiento JIT.

<div id="map-rbac-roles-with-saml-groups">
  ## Mapear roles RBAC con grupos SAML
</div>

Asigna [roles](/es/dashboard/roles) a los usuarios en función de su pertenencia a grupos en el proveedor de identidad. Cuando un usuario inicia sesión a través de SSO, Doxa lee el atributo `groups` de la aserción SAML y mapea esos grupos a roles del dashboard.
...
<Tabs>
  <Tab title="Okta">
    ...
    Ajusta el filtro para que coincida con los grupos específicos que deseas enviar a Doxa.
  </Tab>
...
Una vez configurado, Doxa mapea los nombres de los grupos de la aserción SAML a los roles de tu organización. Para configurar o modificar los mapeos de grupo a rol, contacta a tu representante de cuenta de Doxa.

<div id="change-or-remove-sso-provider">
  ## Cambiar o eliminar el proveedor de SSO
</div>

1. Ve a la página [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso) en tu dashboard.
2. Haz clic en **Configure**.
...
Para proveedores distintos de Microsoft Entra u Okta SAML, [contáctanos](mailto:support@doxa.com) para configurar SSO.
...
<Steps>
  ...
  <Step title="Envíanos la información de tu IdP">
    Copia la SSO URL, el Entity ID y el certificado x509 proporcionados y envíalos al equipo de Doxa.
  ...
  <Step title="Configurar la integración">
    ...
    * ACS URL (proporcionado por Doxa)
    * Entity ID (proporcionado por Doxa)
...
<Steps>
  ...
  <Step title="Configure integration">
    Selecciona el tipo de concesión Authorization Code y proporciona el Redirect URI indicado por Doxa.
  </Step>
