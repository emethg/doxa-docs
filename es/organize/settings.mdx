...
description: "Configura tu sitio de documentación Doxa con docs.json, el archivo que controla la navegación, la apariencia y las integraciones."
keywords: ["docs.json", "configuration", "setup", "mint.json"]
---

El archivo `docs.json` es el archivo de configuración central de tu sitio de documentación Doxa. Controla la configuración global de tu sitio, incluyendo la identidad visual, la estructura de navegación, las integraciones, la configuración de la API y más. Piensa en él como el plano de tu sitio.
...
Agrega una propiedad `$ref` con una ruta de archivo relativa en cualquier lugar de tu `docs.json`. Doxa reemplaza el objeto `$ref` con el contenido del archivo referenciado.
...
Si un `$ref` resuelve a un objeto, Doxa fusiona las claves hermanas del mismo bloque sobre el contenido referenciado, permitiendo que esas claves tengan prioridad sobre las claves coincidentes en la referencia. Si un `$ref` resuelve a un valor que no es un objeto, como un array, Doxa ignora las claves hermanas.
