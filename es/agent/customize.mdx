...
Crea un archivo `.doxa/AGENTS.md` en la raíz de tu proyecto de documentación para personalizar el comportamiento del agente (también se acepta `Agents.md` como nombre de archivo).

<Warning>
  Coloca tu archivo `AGENTS.md` dentro del directorio `.doxa/`, no en la raíz de tu proyecto. Si almacenas el archivo en la raíz de tu proyecto, tus instrucciones personalizadas serán accesibles públicamente a través de la ruta `/agents.md`. El directorio `.doxa/` no se sirve públicamente, por lo que tu archivo `AGENTS.md` no será accesible cuando esté en ese directorio.
