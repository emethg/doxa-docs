...
Add a `.doxa/Assistant.md` file at the root of your documentation project to give the assistant custom instructions that shape how it responds (`ASSISTANT.md` is also an accepted filename). The assistant uses these instructions as system-level context for every response.

<Warning>
  Place your `Assistant.md` file inside the `.doxa/` directory, not at the root of your project. If you store the file at the root of your project, your custom instructions are publicly accessible by the `/assistant.md` path. The `.doxa/` directory is not served publicly so your `Assistant.md` file is not accessible when in that directory.
