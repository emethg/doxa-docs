...
1. Navigate to the [Custom domain setup](https://app.doxa.com/settings/deployment/custom-domain) page in your dashboard.
...
```text
CNAME | docs | cname.doxa.builders
```
...
<Warning>
  ...
  If you migrate an existing domain and want zero downtime, publish the verification `TXT` records first and wait until they show verified and TLS has pre-provisioned before pointing `CNAME` at Doxa.
</Warning>
...
Once your DNS records are active, your documentation is first accessible via HTTP. HTTPS is available after Doxa provisions your TLS certificate.

### Automatic TLS provisioning

After you add your `TXT` records and your DNS records resolve correctly, Doxa generates a free SSL/TLS certificate for your site using Let's Encrypt.
...
Doxa reserves the `/.well-known/acme-challenge` path for certificate validation. You cannot redirect or rewrite this path. If you have configured redirects or rewrites for this path, certificate provisioning fails.
...
Replace `https://www.your-custom-domain-here.com` with your actual custom domain. For example, if your custom domain is `docs.doxa.com`, you would use:

```json
"seo": {
    "metatags": {
        "canonical": "https://docs.doxa.com"
    }
}
```
