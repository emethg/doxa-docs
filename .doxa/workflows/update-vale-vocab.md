...
5. Compare against existing entries in `.vale/styles/config/vocabularies/Doxa/accept.txt` to find words not already present.
6. For each new word, use judgment to determine whether it belongs in the vocabulary:
   - Add to `accept.txt` if it's a valid technical term, product name, or proper noun used in Doxa docs
