...
<Info>
  SSO is available on [Enterprise plans](https://doxa.com/pricing?ref=sso).
</Info>

Enterprise admins can configure SAML SSO for Okta or Microsoft Entra directly from the Doxa dashboard. For other providers like Google Workspace or Okta OIDC, [contact us](mailto:support@doxa.com) to set up SSO.
...
<Steps>
  <Step title="Configure Okta SSO in your Doxa dashboard">
    1. In your Doxa dashboard, navigate to the [Single Sign-On](https://app.doxa.com/settings/organization/sso) page.
    2. Click **Configure**.
    3. Select **Okta SAML**.
  ...
  <Step title="Create a SAML app in Okta">
    ...
    2. Enter the following from Doxa:
        * **Single sign on URL**: the URL you copied from your Doxa dashboard
        * **Audience URI**: the URI you copied from your Doxa dashboard
  ...
  <Step title="Save in Doxa">
    Back in the Doxa dashboard, paste the metadata URL and click **Save changes**.
  </Step>
</Steps>
...
<Steps>
  <Step title="Configure Microsoft Entra SSO in your Doxa dashboard">
    1. In your Doxa dashboard, navigate to the [Single Sign-On](https://app.doxa.com/settings/organization/sso) page.
    2. Click **Configure**.
  ...
  <Step title="Configure SAML in Microsoft Entra">
    ...
    3. Under **Basic SAML Configuration**, enter the following:
        * **Identifier (Entity ID)**: the Audience URI from Doxa
        * **Reply URL (Assertion Consumer Service URL)**: the Single sign on URL from Doxa
  ...
  <Step title="Assign users">
    In Microsoft Entra, navigate to **Users and groups**. Assign the users who should have access to your Doxa dashboard.
  </Step>
</Steps>

## JIT provisioning

When you enable JIT (just-in-time) provisioning, users who sign in through your identity provider are automatically added to your Doxa organization.

<Note>
  JIT provisioning only works for IdP-initiated login. Users must sign in from your identity provider (Okta dashboard or Microsoft Entra portal) rather than starting from the Doxa login page.
</Note>

To enable JIT provisioning, you must have SSO enabled. Navigate to the [Single Sign-On](https://app.doxa.com/settings/organization/sso) page in your dashboard, set up SSO, and then enable JIT provisioning.

## Map RBAC roles with SAML groups

Assign [roles](/dashboard/roles) to users based on their identity provider group membership. When a user signs in through SSO, Doxa reads the `groups` attribute from the SAML assertion and maps those groups to dashboard roles.
...
<Tabs>
  <Tab title="Okta">
    ...
    Adjust the filter to match the specific groups you want to send to Doxa.
  </Tab>
...
Once configured, Doxa maps the group names from the SAML assertion to roles in your organization. To set up or modify group-to-role mappings, reach out to your Doxa account representative.

## Change or remove SSO provider

1. Navigate to the [Single Sign-On](https://app.doxa.com/settings/organization/sso) page in your dashboard.
2. Click **Configure**.
...
For providers other than Microsoft Entra or Okta SAML, [contact us](mailto:support@doxa.com) to configure SSO.
...
<Steps>
  ...
  <Step title="Send us your IdP information">
    Copy the provided SSO URL, Entity ID, and x509 certificate and send it to the Doxa team.
    <Frame>
  ...
  <Step title="Configure integration">
    ...
    * ACS URL (provided by Doxa)
    * Entity ID (provided by Doxa)
...
<Steps>
  ...
  <Step title="Configure integration">
    Select the authorization code grant type and enter the Redirect URI provided by Doxa.
  </Step>
  <Step title="Send us your IdP information">
