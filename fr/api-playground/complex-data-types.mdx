...
description: "Décrivez des API avec des schémas flexibles en utilisant les mots-clés oneOf, anyOf et allOf pour les propriétés optionnelles, le polymorphisme et les formats de données multiples."
keywords: ["oneOf", "anyOf", "allOf", "composition de schémas", "schémas polymorphes"]
---
...
<Warning>Doxa traite `oneOf` et `anyOf` de la même manière, car la différence pratique influe rarement sur l’utilisation de l’API.</Warning>
...
Lorsque vous utilisez `allOf`, Doxa applique un prétraitement à votre document OpenAPI afin d’afficher des combinaisons complexes de manière lisible. Par exemple, lorsque vous combinez deux schémas d’objet avec `allOf`, Doxa fusionne les propriétés des deux en un seul objet. Cela est particulièrement utile lorsque vous exploitez les [components](https://swagger.io/docs/specification/components/) réutilisables d’OpenAPI.
