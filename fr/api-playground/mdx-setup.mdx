...
description: "Créez manuellement des pages de référence d'API avec des fichiers MDX pour un contrôle total sur la mise en page des petites API, prototypes ou docs personnalisées."
...
<Steps>
  ...
  <Step title="Créez les pages de vos endpoints">
    ...
    * **Chemin relatif** comme `POST /v1/users`. Nécessite un champ `server` dans `docs.json`. L’URL du serveur est préfixée au chemin.
    ...
    ---
    title: 'Créer un nouvel utilisateur'
    api: 'POST https://api.doxa.com/user'
    playground: 'none'
    ---
    ```
