...
description: "Générez une documentation API interactive à partir de fichiers de spécification OpenAPI avec des pages d'endpoints automatiques, des constructeurs de requêtes et une navigation."
keywords: ["OpenAPI", "API specification", "Swagger"]
---

OpenAPI est une spécification destinée à décrire des API. Doxa prend en charge les documents OpenAPI 3.0 et 3.1 pour générer une documentation d’API interactive et la maintenir à jour.
...
<Note>
  Doxa prend en charge `$ref` pour les **références internes uniquement** au sein d'un seul document OpenAPI. Les références externes ne sont pas prises en charge.
</Note>
...
Ajoutez du contenu avant la documentation API générée automatiquement à l’aide de `x-mint: content`. L’extension `x-mint: content` est compatible avec tous les composants MDX de Doxa ainsi que leur mise en forme.
...
* `title` : le champ `summary` de l’opération, s’il est présent. S’il n’y a pas de `summary`, Doxa génère le titre à partir de la méthode HTTP et de l’endpoint.
...
<Note>
  Le champ `directory` est facultatif et indique où Doxa stocke les pages d’API générées dans votre dépôt de documentation. S’il n’est pas défini, le répertoire par défaut est `api-reference` à la racine de votre dépôt.
</Note>
...
Pour générer automatiquement des fichiers MDX à partir de votre spécification OpenAPI, utilisez le [scraper](https://www.npmjs.com/package/@doxa/scraping) de Doxa.
