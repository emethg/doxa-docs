...
description: "Configurez l'authentification utilisateur pour votre site avec OAuth, JWT ou des liens partagés afin de contrôler l'accès aux pages et aux références d'API."
keywords: ['authentication', 'auth', 'OAuth', 'JWT', 'password']
---

<Info>
  L'authentification au tableau de bord Doxa est disponible sur toutes les offres.

  L'authentification par mot de passe, OAuth et JWT nécessite une [offre Enterprise](https://doxa.com/pricing?ref=authentication).
</Info>

<Warning>
  L'authentification n'est disponible que pour les sites hébergés sur un domaine personnalisé ou un sous-domaine Doxa (par exemple, `docs.exemple.com` ou `exemple.doxa.dev`). L'authentification **n'est pas prise en charge** lors de l'utilisation d'un [chemin de base personnalisé](/fr/deploy/docs-subpath) (par exemple, `exemple.com/docs`).
</Warning>
...
<Tabs>
    ...
    * Vos exigences de sécurité autorisent le partage de mots de passe entre les utilisateurs.
    ...
    <Steps>
      <Step title="Créer un mot de passe.">
        1. Dans votre Dashboard, accédez à [Authentication](https://dashboard.doxa.com/products/authentication).
  ...
  <Tab title="Tableau de bord Doxa">
    ### Prérequis du tableau de bord Doxa

    * Toutes les personnes qui doivent accéder à votre documentation doivent être membres de votre organisation Doxa.

    ### Configuration du tableau de bord Doxa

    <Steps>
      <Step title="Activer l’authentification via le Tableau de bord Doxa.">
        1. Dans votre Dashboard, accédez à [Authentication](https://dashboard.doxa.com/products/authentication).
        2. Activez l’authentification.
        3. Dans la section **Custom Authentication**, cliquez sur **Doxa Auth**.
        4. Cliquez sur **Enable Doxa Auth**.

        Après avoir activé l’authentification Doxa, votre site est redéployé. Lorsque le déploiement est terminé, toute personne qui visite votre site doit se connecter à votre organisation Doxa pour accéder à votre contenu.
      </Step>

      <Step title="Ajouter des utilisateurs autorisés.">
        1. Dans votre Tableau de bord Doxa, allez à [Members](https://dashboard.doxa.com/settings/organization/members).
    ...
    ### Exemple du tableau de bord Doxa

    Vous hébergez votre documentation sur `docs.foo.com` et toute votre équipe a accès à votre Tableau de bord Doxa. Vous souhaitez restreindre l’accès aux seuls membres de l’équipe.

    **Activez l’authentification Doxa** dans les paramètres de votre Tableau de bord Doxa.
    ...
    * Capacité à créer un endpoint d&#39;API accessible via des jetons d&#39;accès OAuth (facultatif, pour activer le contrôle d&#39;accès basé sur les groupes).
    ...
    <Steps>
      <Step title="Configurez vos paramètres OAuth.">
        1. Dans votre Dashboard, allez dans [Authentication](https://dashboard.doxa.com/products/authentication).
        2. Activez l’authentification.
        ...
        * **Info API URL** (facultatif) : Endpoint sur votre serveur que Doxa appelle pour récupérer les informations utilisateur. Requis pour le contrôle d’accès basé sur les groupes. S’il est omis, le flux OAuth vérifie uniquement l’identité.
        * **Logout URL** (facultatif) : L’URL de déconnexion native de votre fournisseur OAuth. Lorsque les utilisateurs se déconnectent, Doxa valide la redirection de déconnexion par rapport à l’URL configurée, pour des raisons de sécurité. La redirection ne réussit que si elle correspond exactement à la valeur de `logoutUrl` configurée. Si vous ne configurez pas d’URL de déconnexion, les utilisateurs sont redirigés vers `/login`. Doxa redirige les utilisateurs avec une requête `GET` et n’ajoute aucun paramètre de requête. Incluez donc directement tous les paramètres (par exemple, `returnTo`) dans l’URL.
      ...
      <Step title="Configurez votre serveur OAuth.">
        1. Copiez la **Redirect URL** à partir de vos [paramètres d’authentification](https://dashboard.doxa.com/products/authentication).
      ...
      <Step title="Créez votre endpoint d’informations utilisateur (facultatif).">
        ...
        * Renvoie les données utilisateur au format `User`. Voir [User data format](#user-data-format) pour plus d’informations.
        ...
        Doxa appelle cet endpoint avec le jeton d’accès OAuth pour récupérer les informations utilisateur. Aucun paramètre de requête supplémentaire n’est envoyé.

        Ajoutez l’URL de cet endpoint dans le champ **Info API URL** de vos [paramètres d’authentification](https://dashboard.doxa.com/products/authentication).
      </Step>
    </Steps>
  ...
  <Tab title="JWT">
    ...
    * Un service backend capable de créer des URL de redirection.
    ...
    <Steps>
      ...
      <Step title="Intégrez l’authentification Doxa dans votre flux de connexion.">
    ...
    * Générez un JWT avec les données utilisateur au format de Doxa.
    * Signez le JWT et redirigez vers `https://docs.foo.com/login/jwt-callback#{SIGNED_JWT}`.
    ...
    <CodeGroup>
      ...
      const signingKey = await jose.importPKCS8(process.env.DOXA_PRIVATE_KEY, 'EdDSA');
      ...
      private_key = os.getenv(DOXA_JWT_PEM_SECRET_NAME, '')
      DOCS_HOST = 'docs.example.com'

      @router.get('/auth')
      async def return_doxa_auth_status(current_user):
...
<ParamField path="host" type="string">
  **Requis pour l’authentification JWT.** Le nom d’hôte de votre site de documentation. La chaîne doit correspondre exactement au domaine sur lequel vous déployez votre documentation. Doxa valide que l’hôte du JWT correspond à l’hôte de la requête afin d’empêcher la réutilisation du jeton sur différents sites.
</ParamField>
...
Certaines fonctionnalités se comportent différemment ou ne sont pas disponibles lorsque vous activez l’authentification. Doxa ne prend pas en charge l’hébergement de fichiers publics arbitraires sur un site authentifié. Tous les fichiers hébergés, y compris `llms.txt`, `llms-full.txt` et `skill.md`, sont soumis aux mêmes exigences d’authentification que vos pages de documentation.
