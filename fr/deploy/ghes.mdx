...
description: "Installez et configurez l’application GitHub Doxa sur votre instance GitHub Enterprise Server pour des déploiements automatiques de documentation et la synchronisation."
keywords: ["GitHub Enterprise Server", "GHES", "GitHub Enterprise"]
---

Ce guide vous explique comment configurer l’App GitHub Doxa sur votre installation GitHub Enterprise Server (GHES). Pour connecter une instance GHES à Doxa, vous devez créer une version locale de notre application au sein de votre environnement auto‑hébergé, qui communique avec notre serveur distant.
...
* les points de terminaison de l&#39;API Doxa (https://leaves.doxa.com)
...
* Connexions provenant de l&#39;adresse IP statique de Doxa : `54.242.90.151`
* HTTPS (port 443) vers les domaines de service de Doxa
* Résolution DNS pour les domaines de service de Doxa
...
<Steps>
  ...
  <Step title="Configurer les informations de base de l'application">
    ...
    * **GitHub App name :** `Doxa`
    * **Description :** `Intégration avec les services Doxa`
    * **Homepage URL :** `https://doxa.com`
...
<Steps>
  ...
  <Step title="Définir l’URL du webhook">
    ...
    * **Webhook URL :** `https://leaves.doxa.com/github-enterprise/:subdomain` (remplacez `:subdomain` par l’URL que nous vous fournirons)
    * **Webhook secret :** Générez une chaîne de caractères aléatoire d’au moins 32 caractères et conservez-la en lieu sûr. Doxa peut également la générer et vous la fournir.
  </Step>
</Steps>
...
* Intégrer votre environnement GHES à un déploiement Doxa existant.
* Intégrer votre environnement GHES à un nouveau déploiement Doxa que nous mettons en place pour vous.

Une fois que votre environnement GHES est intégré à un déploiement Doxa, vous êtes prêt à activer les webhooks pour votre GitHub App.
...
<Steps>
  ...
  <Step title="Tester l’accès au dépôt">
    ...
    2. Vérifiez que Doxa réagit comme attendu.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Qui dois-je contacter si j'ai besoin d'aide ?">
    Veuillez contacter votre représentant Customer Success avec qui vous avez échangé chez Doxa, ou notre équipe support à l&#39;adresse <a href="mailto:support@doxa.com">support@doxa.com</a> avec :
