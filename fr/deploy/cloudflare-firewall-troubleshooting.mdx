...
description: "Résolvez les requêtes bloquées et les conflits de règles WAF Cloudflare lors de la diffusion de la documentation Doxa via Cloudflare Workers."
keywords: ["Cloudflare WAF", "règles de pare-feu", "Bot Fight Mode", "erreurs 403"]
---

Si votre site de documentation affiche des erreurs 500 après quelques secondes ou rencontre une navigation lente, le pare-feu de Cloudflare bloque peut-être les requêtes vers les ressources Doxa.
...
- Erreurs 403 dans la console du navigateur pour les requêtes vers les chemins `/doxa-assets/*`
...
Le pare-feu d’applications web (WAF) de Cloudflare et le Bot Fight Mode peuvent considérer les requêtes d’actifs Doxa comme suspectes en raison de :
...
Créez une règle de pare-feu Cloudflare pour exclure les ressources Doxa des contrôles de sécurité.
...
**Nom de la règle :** Autoriser les ressources Doxa
...
- Utiliser l’opérateur `contains` avec `/doxa-assets/*` - Le `*` est interprété comme un caractère littéral, pas comme un caractère générique
- Utiliser `equals` pour le chemin URI - Cela ne fait correspondre que le chemin exact `/doxa-assets/` et non les sous-chemins
...
```
Règle : Autoriser les ressources Doxa
Statut : Activé

Lorsque les requêtes entrantes correspondent à :
  (http.host eq "docs.flashnet.xyz" and starts_with(http.request.uri.path, "/doxa-assets/"))
