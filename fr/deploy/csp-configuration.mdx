...
description: "Configurez les en-têtes Content Security Policy pour autoriser les ressources Doxa tout en maintenant la sécurité pour les reverse proxies et les réseaux stricts."
keywords: ["Content Security Policy", "CSP", "directives", "en-têtes de sécurité", "pare-feu", "prévention XSS"]
---

La Content Security Policy (CSP) est une norme de sécurité qui aide à prévenir les attaques de cross-site scripting (XSS) en contrôlant les ressources qu’une page web est autorisée à charger. Doxa fournit une CSP par défaut qui protège la plupart des sites. Si vous hébergez votre documentation derrière un reverse proxy ou un pare-feu qui remplace la CSP par défaut, vous devrez peut-être configurer les en-têtes CSP pour que certaines fonctionnalités fonctionnent correctement.
...
| `*.doxa.dev` | Contenu de la documentation | `connect-src`, `frame-src` | Obligatoire |
| `*.doxa.com` | Dashboard, API, proxy d’Analytics | `connect-src` | Obligatoire |
| `leaves.doxa.com` | API Assistant | `connect-src` | Obligatoire |
...
| `doxa.s3.us-west-1.amazonaws.com` | Images hébergées sur S3 | `img-src` | Obligatoire |
