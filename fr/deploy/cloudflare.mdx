---
title: "Cloudflare"
description: "Déployez votre documentation Doxa sur un sous-chemin de votre domaine à l'aide de Cloudflare Workers, avec une configuration pas à pas et des paramètres DNS."
...
Assurez-vous que l’en-tête `HOST` est correctement transféré dans la configuration de votre Worker. Un transfert incorrect des en-têtes entraîne l’échec des requêtes de vérification.
...
async function handleRequest(request) {
  try {
    ...
    if (/^\/docs/.test(urlObject.pathname)) {
      // Alors rediriger via proxy vers Doxa
...
Après le déploiement de votre code, testez votre Worker pour vérifier qu’il redirige vers votre documentation Doxa.

1. Testez en utilisant l’URL d’aperçu du Worker : `your-worker.your-subdomain.workers.dev/docs`
2. Vérifiez que le Worker redirige vers votre documentation Doxa et votre site web.
...
Si vous utilisez Webflow pour héberger votre site principal et que vous souhaitez servir la documentation Doxa à `/docs` sur le même domaine, vous devrez configurer un routage personnalisé via Cloudflare Workers pour faire transiter (proxy) tout le trafic non lié à la documentation vers votre site principal.

<Warning>
  Assurez-vous que votre site principal est configuré sur une page d’atterrissage avant de déployer ce Worker, sinon les visiteurs de votre site principal pourraient voir des erreurs.
</Warning>
