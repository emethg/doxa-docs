...
description: "Formatez le texte dans votre documentation avec des titres Markdown, du gras, de l'italique, des liens, des citations et d'autres options de style en ligne dans les pages MDX."
keywords: ["Mise en forme Markdown", "style du texte", "titres", "liens d’ancrage", "custom heading IDs"]
---
...
<div id="custom-heading-ids">
  ### IDs de titre personnalisés
</div>

Par défaut, Doxa génère automatiquement les IDs d'ancrage à partir du texte du titre. Vous pouvez remplacer l'ID généré par un ID personnalisé en utilisant la syntaxe `{#custom-id}` :
...
L'ID personnalisé remplace l'ancrage généré automatiquement. Vous pouvez ainsi créer un lien vers le titre avec `#my-custom-anchor` ou `#config` au lieu du texte slugifié par défaut.
...
Vous pouvez vérifier la présence de liens brisés dans votre documentation à l’aide de l’interface en ligne de commande (CLI) : [CLI](/fr/cli)
...
> Ce texte se démarque du contenu principal.
