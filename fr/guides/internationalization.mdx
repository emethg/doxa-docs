...
Pour des solutions de traduction automatique, [configurez un workflow](/workflows) pour exécuter l'agent selon un calendrier ou en réponse à des pushes vers le dépôt.

<div id="external-translation-providers">
  ### Prestataires de traduction externes
</div>

Si vous travaillez avec vos propres prestataires de traduction ou traducteurs régionaux, vous pouvez intégrer leur processus à votre documentation Doxa à l’aide de GitHub Actions ou d’outils CI/CD similaires.
...
<Accordion title="Que se passe-t-il lorsqu’une page traduite est manquante ?">
...
Oui. Les libellés de navigation — noms de groupes, titres d’onglets, textes d’ancrage — doivent correspondre à la langue du contenu. Un libellé « Getting started » en anglais dans une section de documentation en espagnol crée une expérience incohérente. Doxa prend en charge les structures de navigation spécifiques à chaque langue, de sorte que chaque langue peut avoir des libellés entièrement traduits.
</Accordion>

<Accordion title="Comment gérer les exemples de code dans le contenu traduit ?">
Le code lui-même ne doit pas être traduit — les noms de variables, les appels de fonctions et la syntaxe sont indépendants de la langue. Les commentaires dans les code blocks peuvent être traduits s’ils expliquent des concepts que les utilisateurs doivent comprendre. Les instructions entourant les code blocks doivent être entièrement traduites.
</Accordion>
</AccordionGroup>