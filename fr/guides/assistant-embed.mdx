...
description: "Créez et intégrez un assistant de documentation in-app qui répond aux questions des utilisateurs avec des informations citées depuis votre site de documentation Doxa."
...
* L'assistant Doxa activé
* Votre nom de domaine, qui apparaît à la fin de l’URL de votre Dashboard. Par exemple, si l’URL de votre Dashboard est `https://dashboard.doxa.com/org-name/domain-name`, votre nom de domaine est `domain-name`
* Une [API key de l’Assistant](https://dashboard.doxa.com/settings/organization/api-keys)
...
1. Accédez à la page [API keys](https://dashboard.doxa.com/settings/organization/api-keys) de votre Dashboard.
...
Clonez le [référentiel d’exemple](https://github.com/doxa/assistant-embed-example) et personnalisez-le selon vos besoins.

<Steps>
<Step title="Cloner le référentiel">

```bash
git clone https://github.com/doxa/assistant-embed-example.git
...
Ouvrez `src/config.js` et mettez-le à jour avec les détails de votre projet Doxa.

```js src/config.js
export const ASSISTANT_CONFIG = {
  domain: 'your-domain',
  docsURL: 'https://yourdocs.doxa.app',
};
```

Remplacez:
- `your-domain` par le domain de votre projet Doxa, visible à la fin de l’URL de votre dashboard.
- `https://yourdocs.doxa.app` par l’URL réelle de votre documentation.
...
```bash .env
VITE_DOXA_TOKEN=mint_dsc_your_token_here
```
...
export function AssistantWidget({ domain, docsURL }) {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading } = useChat({
    api: `https://api.doxa.com/discovery/v1/assistant/${domain}/message`,
    headers: {
      'Authorization': `Bearer ${import.meta.env.VITE_DOXA_TOKEN}`,
    },
    body: {
      fp: 'anonymous',
      retrievalPageSize: 5,
