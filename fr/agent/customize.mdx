...
description: "Personnalisez le comportement de l'agent avec un fichier de configuration `AGENTS.md` pour contrôler comment il gère les tâches de documentation et suit vos conventions."
keywords: ["AGENTS.md", "configuration de l'agent", "instructions personnalisées", "personnalisation de l'agent"]
---

Créez un fichier `.doxa/AGENTS.md` à la racine de votre projet de documentation pour personnaliser le comportement de l'agent (`Agents.md` est également accepté comme nom de fichier).

<Warning>
  Placez votre fichier `AGENTS.md` dans le répertoire `.doxa/`, et non à la racine de votre projet. Si vous stockez le fichier à la racine de votre projet, vos instructions personnalisées seront accessibles publiquement via le chemin `/agents.md`. Le répertoire `.doxa/` n'est pas servi publiquement, donc votre fichier `AGENTS.md` ne sera pas accessible lorsqu'il se trouve dans ce répertoire.
