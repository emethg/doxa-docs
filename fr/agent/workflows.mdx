...
1. Ouvrez la page [Workflows](https://dashboard.doxa.com/products/workflows).
...
Pour les référentiels GitHub, vous devez avoir installé la GitHub App Doxa sur chaque référentiel répertorié dans les champs de contexte ou de déclencheur. Ajoutez de nouveaux référentiels sur la page [GitHub app](https://dashboard.doxa.com/settings/organization/github-app) de votre dashboard Doxa.

Pour les référentiels GitLab, vous devez connecter votre compte GitLab sur la page [GitLab OAuth](https://dashboard.doxa.com/settings/organization/gitlab-oauth) de votre dashboard Doxa. Ajoutez ensuite les référentiels que vous souhaitez utiliser avec les workflows. Vous devez disposer au minimum du rôle Maintainer sur chaque projet pour connecter GitLab OAuth.
...
<Note>
  Pour les référentiels GitHub, l'automerge nécessite que la GitHub App Doxa dispose de permissions de contournement sur tous les ensembles de règles ciblant votre branche de déploiement, y compris les ensembles de règles définis au niveau de l'organisation et au niveau du dépôt. Consultez [Configurer l'automerge](/fr/guides/configure-automerge) pour les instructions de configuration.

  Pour les référentiels GitLab, l'automerge utilise la connexion GitLab OAuth configurée sur la page [GitLab OAuth](https://dashboard.doxa.com/settings/organization/gitlab-oauth) de votre dashboard Doxa. Vous devez disposer au minimum du rôle Maintainer sur chaque projet pour connecter GitLab OAuth.
</Note>
...
<Note>
Les notifications Slack nécessitent l'application Doxa Slack dans l'espace de travail Slack de votre organisation. Installez l'application Slack depuis votre [dashboard](https://dashboard.doxa.com/products/agent).
</Note>
...
Modifiez, supprimez, désactivez ou déclenchez des workflows depuis la page [Workflows](https://dashboard.doxa.com/products/workflows) de votre dashboard.
...
1. Accédez à la page [Workflows](https://dashboard.doxa.com/products/workflows) dans votre dashboard.
...
Lorsque vous réactivez un workflow basé sur cron, Doxa recalcule le prochain délai d'exécution à partir de l'heure actuelle.
