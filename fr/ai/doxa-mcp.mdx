...
description: "Permettez aux outils d'IA de modifier directement votre contenu et de mettre à jour les paramètres de votre tableau de bord avec le serveur Admin MCP. Connectez Claude, Cursor ou tout autre client MCP pour rédiger, enregistrer et publier des changements."
...
| **Endpoints** | Hébergé par Doxa, à la portée de votre projet | `/mcp` sur le domaine de votre site |
...
Vous devez disposer d'une connexion OAuth interactive à votre compte Doxa pour vous connecter à l'Admin MCP. Les outils d'IA échangent cette connexion contre un jeton de session limité à un seul projet.
...
<Tabs>
    ...
    <Steps>
      <Step title="Ajouter l'Admin MCP comme connecteur personnalisé">
        ...
        3. Ajoutez le connecteur
           - Nom : Admin MCP
           - URL : `https://mcp.doxa.com`
        4. Cliquez sur **Add** et terminez la connexion OAuth.
      </Step>
      <Step title="Utiliser le MCP dans une conversation">
        Cliquez sur le bouton des pièces jointes (l'icône plus), puis sélectionnez votre serveur Admin MCP. Claude peut maintenant appeler les outils Doxa Admin MCP tout en répondant à votre prompt.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Claude Code">
    ...
    ```bash
    claude mcp add --transport http doxa https://mcp.doxa.com
    ```
...
<Steps>
  <Step title="Extraire une branche">
    Le premier appel doit être `checkout`. Il crée une nouvelle branche `doxa-mcp/<slug>-<sha>` à partir de votre branche de déploiement (ou se rattache à une branche existante que vous nommez) et renvoie une `editorUrl` que vous pouvez ouvrir pour suivre l'évolution dans l'éditeur du tableau de bord.
...
<Note>
  Les sessions conservent une branche en mémoire côté Doxa. Si vous abandonnez une session sans l'enregistrer ou la supprimer, la branche persiste jusqu'à ce que votre prochain checkout l'écrase. Évitez de laisser des branches `doxa-mcp/*` obsolètes dans votre dépôt. Nettoyez-les périodiquement.
</Note>
