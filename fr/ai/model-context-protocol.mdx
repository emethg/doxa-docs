...
description: "Connectez votre contenu aux outils d'IA comme Claude et Cursor avec un serveur MCP hébergé pour la recherche."
...
<Tip>
  Vous souhaitez plutôt permettre aux agents de modifier votre contenu au lieu de simplement le lire ? Utilisez le [serveur Admin MCP](/fr/ai/doxa-mcp) pour un serveur MCP authentifié qui expose des outils de branching, d'édition de pages, de navigation et de `docs.json` aux agents de confiance.
</Tip>
...
Les ressources `Skill.md` apparaissent dans la liste des ressources du serveur Search MCP et contiennent les descriptions de capacités que Doxa génère ou que vous définissez dans vos [fichiers de skill personnalisés](/fr/ai/skillmd#custom-skill-files).
...
Doxa génère un serveur Search MCP pour votre site et l'héberge au chemin `/mcp` de l'URL de votre site. Par exemple, le serveur Search MCP de Doxa est disponible à l'adresse `https://doxa.com/docs/mcp`.
...
Vous pouvez afficher et copier l'URL de votre serveur Search MCP sur la [page du serveur MCP](https://dashboard.doxa.com/products/mcp) dans votre Dashboard.
...
Doxa héberge un document de découverte à l'adresse `/.well-known/mcp` pour permettre aux agents et aux outils de localiser votre serveur Search MCP sans configuration préalable.
...
Pour la compatibilité avec les agents, le même document de découverte est également disponible à l’adresse `/.well-known/mcp.json`. Doxa expose en plus une carte de serveur MCP à `/.well-known/mcp/server-card.json` et une liste de cartes de serveur à `/.well-known/mcp/server-cards.json`. Tous les endpoints de découverte sont servis automatiquement et ne nécessitent aucune configuration.
...
<Steps>
  <Step title="Activer le MCP avec authentification dans votre Dashboard">
    1. Accédez à la [page du serveur MCP](https://dashboard.doxa.com/products/mcp) dans votre Dashboard.
...
Vous pouvez gérer vos identifiants client depuis la [page du serveur MCP](https://dashboard.doxa.com/products/mcp) dans votre Dashboard.
...
Pour protéger la disponibilité, Doxa applique des limites de débit aux serveurs MCP.
...
Votre serveur MCP recherche le contenu que Doxa indexe à partir du référentiel de votre projet. Le traitement des fichiers et l'indexation pour la recherche déterminent quel contenu est disponible via votre serveur MCP.
...
Si des fichiers correspondent aux modèles définis dans [.mintignore](/fr/organize/mintignore), Doxa ne les traite pas et ne les indexe pas. Ces fichiers ne sont pas disponibles via votre serveur MCP.

<div id="search-indexing-with-docsjson">
  ### Indexation pour la recherche avec `docs.json`
</div>

Par défaut, Doxa n&#39;indexe, pour la recherche via votre serveur MCP, que les pages incluses dans la navigation de votre `docs.json`.

Doxa exclut les [pages masquées](/fr/organize/hidden-pages) (pages qui ne figurent pas dans votre navigation) de l&#39;index de recherche, sauf si vous choisissez d&#39;indexer toutes les pages. Pour inclure les pages masquées dans les résultats de recherche de votre serveur MCP, ajoutez la propriété `seo.indexing` à votre `docs.json`.
  ...
  <Tab title="Claude">
    <Steps>
      <Step title="Obtenez l'URL de votre serveur MCP.">
        Accédez à votre [Dashboard](https://dashboard.doxa.com/products/mcp) et trouvez l&#39;URL de votre serveur MCP.
      </Step>
...
<div id="example-connect-to-the-doxa-mcp-server">
  ### Exemple : connexion au serveur MCP de Doxa
</div>

Connectez-vous au serveur MCP de Doxa pour rechercher sur ce site de documentation depuis votre outil d&#39;IA préféré. Vous obtiendrez ainsi des réponses plus précises sur l&#39;utilisation de Doxa dans votre environnement local, et vous verrez comment aider vos utilisateurs à se connecter à votre serveur MCP.

<Tabs>
  <Tab title="Menu contextuel">
    En haut de cette page, ouvrez le menu contextuel et choisissez **Connect to Cursor** ou **Connect to VS Code** pour connecter le serveur MCP de Doxa à l&#39;IDE de votre choix.
  </Tab>

  <Tab title="Claude">
    Pour utiliser le serveur MCP de Doxa avec Claude :

    <Steps>
      <Step title="Ajouter le serveur MCP de Doxa à Claude">
        1. Accédez à la page [Connectors](https://claude.ai/settings/connectors) dans les paramètres de Claude.
        2. Sélectionnez **Add custom connector**.
        3. Ajoutez le serveur MCP de Doxa :
      ...
      <Step title="Accéder au serveur MCP dans votre chat">
        ...
        2. Sélectionnez le serveur MCP de Doxa.
        3. Posez à Claude une question sur Doxa.
      </Step>
    </Steps>
  ...
  <Tab title="Claude Code">
    Pour utiliser le serveur MCP de Doxa avec Claude Code, exécutez la commande suivante :
  ...
  <Tab title="Cursor">
    <PreviewButton href="cursor://anysphere.cursor-deeplink/mcp/install?name=doxa&config=eyJ1cmwiOiJodHRwczovL21pbnRsaWZ5LmNvbS9kb2NzL21jcCJ9">Installer dans Cursor</PreviewButton>

    Pour connecter le serveur MCP de Doxa à Cursor, cliquez sur le bouton **Installer dans Cursor**. Ou, pour le connecter manuellement, suivez ces étapes :
    ...
    <Steps>
      ...
      <Step title="Configurer le serveur MCP de Doxa">
      ...
      <Step title="Tester la connexion">
        Dans le chat de Cursor, demandez « What tools do you have available? ». Cursor doit afficher le serveur MCP de Doxa comme outil disponible.
      </Step>
    </Steps>
  ...
  <Tab title="VS Code">
    <PreviewButton href="https://vscode.dev/redirect/mcp/install?name=doxa&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fdoxa.com%2Fdocs%2Fmcp%22%7D">Installer dans VS Code</PreviewButton>

    Pour connecter le serveur MCP de Doxa à VS Code, cliquez sur le bouton **Installer dans VS Code**. Ou, pour le connecter manuellement, créez un fichier `.vscode/mcp.json` et ajoutez :
