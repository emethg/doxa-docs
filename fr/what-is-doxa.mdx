---
title: "Qu'est-ce que Doxa ?"
description: "Doxa est une plateforme de documentation conçue pour les développeurs et l’IA, avec une recherche intelligente, des playgrounds d’API interactifs et des analytiques intégrées."
keywords: ["vue d'ensemble", "comment ça marche", "architecture"]
---

Doxa héberge votre contenu sous la forme d’un site web. Votre contenu se trouve dans un référentiel Git sous forme de fichiers MDX, et Doxa génère et déploie automatiquement votre site lorsque vous effectuez un push.

<div id="the-three-parts-of-a-doxa-project">
  ## Les trois parties d’un projet Doxa
</div>
...
**Le Tableau de bord Doxa** se connecte à votre référentiel et vous permet de gérer votre site. Utilisez-le pour surveiller les déploiements, configurer les paramètres, gérer votre équipe et modifier le contenu directement dans le navigateur.

**Votre site**, propulsé par Doxa. Doxa génère votre site à partir de votre référentiel et le déploie par défaut sous une URL en `.doxa.app`. Lorsque vous êtes prêt, vous pouvez associer un domaine personnalisé à votre site.
