---
title: "Commandes"
description: "Référence complète de toutes les commandes et options de la CLI Doxa, y compris mint dev, mint build, mint validate, mint analytics et plus encore."
...
S'authentifier avec votre compte Doxa.

```bash
mint login
```

Ouvre une fenêtre de navigateur pour compléter l'authentification. Si le navigateur ne s'ouvre pas, la CLI affiche une URL à ouvrir manuellement et un champ pour coller le code d'autorisation. Les identifiants sont enregistrés dans `~/.config/doxa/config.json`.
...
Gérer les valeurs par défaut persistantes pour les commandes de la CLI. La configuration est enregistrée dans `~/.config/doxa/config.json`.
...
| `skillMd` | Les agents peuvent atteindre un fichier [skill.md](https://www.doxa.com/docs/ai/skillmd) pour l'utilisation d'outils par les agents. |
...
Créer un nouveau projet de documentation en choisissant un thème ou en clonant un modèle prédéfini depuis le dépôt [doxa/templates](https://github.com/doxa/templates).
...
La CLI collecte des données de télémétrie anonymes pour aider à améliorer Doxa. Les données de télémétrie incluent le nom de la commande, la version de la CLI, le système d'exploitation et l'architecture. Doxa ne collecte **pas** d'informations personnellement identifiables, de contenu de projet ni de chemins de fichiers.
...
| --- | --- | --- |
| `DOXA_TELEMETRY_DISABLED` | `1` | Désactiver la télémétrie de la CLI Doxa. |
| `DO_NOT_TRACK` | `1` | Désactiver la télémétrie en utilisant le standard [Console Do Not Track](https://consoledonottrack.com/). |

Votre préférence est enregistrée dans `~/.config/doxa/config.json` et persiste entre les sessions de la CLI.
