...
description: "Installez la CLI Doxa pour prévisualiser la documentation localement, tester les modifications en temps réel et détecter les erreurs de compilation avant le déploiement en production."
...
Pour créer un nouveau projet de documentation à partir du modèle de démarrage Doxa, exécutez la commande suivante :
...
Consultez les modèles disponibles dans le dépôt [doxa/templates](https://github.com/doxa/templates) sur GitHub. En mode interactif, la CLI récupère et affiche automatiquement les modèles disponibles.
...
<AccordionGroup>
  ...
  <Accordion title="Problème : une erreur inconnue se produit">
    **Solution** : Allez à la racine de votre appareil et supprimez le dossier `~/.doxa`. Ensuite, exécutez à nouveau `mint dev`.
  </Accordion>
  ...
  <Accordion title="Package doxa versus mint">
    ...
    Si vous avez à la fois le package `mint` et le package `doxa` installés, désinstallez `doxa` :

    ```bash
    npm uninstall -g doxa
  ...
  <Accordion title="La version du client affiche 'none' après l'installation">
    ...
    **Solution** : Demandez à votre administrateur informatique d'ajouter `releases.doxa.com` à la liste d'autorisation de votre réseau.
  </Accordion>
  <Accordion title="La CLI se connecte à localhost au lieu de la production lorsqu'on utilise npx">
    ...
    des URLs `localhost` au lieu de l'API de production Doxa, ce qui entraînait des
