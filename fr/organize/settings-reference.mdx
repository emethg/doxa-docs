...
- Lorsque `$ref` résout en un objet, Doxa fusionne les clés sœurs du même bloc par-dessus le contenu référencé, permettant à ces clés de prendre le dessus sur les clés correspondantes de la référence.
- Lorsque `$ref` résout en une valeur non-objet telle qu'un tableau, Doxa ignore les clés sœurs.
...
Indique s'il faut charger les feuilles de style LaTeX. Par défaut, Doxa détecte automatiquement l'utilisation de LaTeX.
...
Clés de champs de la spécification OpenAPI à faire apparaître sous forme de pastilles post à côté du nom de chaque paramètre. Pour chaque clé, Doxa lit la valeur sur le schéma et l'affiche sous forme de pastille : les chaînes s'affichent telles quelles, `true` affiche le nom de la clé, les nombres sont convertis en chaîne, et les tableaux affichent une pastille par élément. Doxa ignore `false`, `null`, les chaînes vides et les objets.
...
| `integrations.telemetry.enabled` | boolean | Non | Activer la télémétrie Doxa. Lorsque `false`, les fonctionnalités de feedback sont également désactivées. |
