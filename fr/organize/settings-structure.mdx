...
<ResponseField name="navbar.links" type="array of object">
  ...
  <Expandable title="navbar.links">
    ...
    <ResponseField name="label" type="string">
      Texte du lien. Requis lorsque `type` n'est pas défini. Facultatif pour `github` et `discord`. Si omis, Doxa génère le libellé à partir des données de l'API.
    </ResponseField>
    <ResponseField name="href" type="string (uri)" required>
...
Variables globales à utiliser dans toute votre documentation. Doxa remplace les espaces réservés `{{variableName}}` par les valeurs définies au moment de la compilation.
...
<ResponseField name="variables.[variableName]" type="string">
  ...
  - Doxa assainit les valeurs pour prévenir les attaques XSS.
</ResponseField>
