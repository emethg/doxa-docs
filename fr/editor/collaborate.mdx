...
description: "Modifiez la doc en équipe avec la collaboration en temps réel, les curseurs en direct, les liens d'éditeur partageables, les branches et les pull requests."
...
2. Cliquez sur l’URL de prévisualisation pour ouvrir le déploiement de prévisualisation dans un nouvel onglet. Le format de l’URL est `organization-branch-name.doxa.app`.
...
Les URL de prévisualisation sont accessibles publiquement par défaut. Activez l’authentification de prévisualisation sur la page [Modules complémentaires](https://dashboard.doxa.com/products/addons) de votre dashboard pour restreindre l’accès aux membres authentifiés de l’organisation.
...
Le format de l’URL est : `https://dashboard.doxa.com/{org}/{project}/editor/{branch}/~/{filepath}`

Par exemple, `https://dashboard.doxa.com/acme/docs/editor/main/~/guides/quickstart.mdx` ouvre le fichier `quickstart.mdx` sur la branche `main`.

Toute personne ayant accès à votre organisation Doxa peut utiliser le lien pour ouvrir le fichier directement dans l’éditeur.
