...
Doxa prend automatiquement en charge de nombreuses bonnes pratiques SEO, notamment :
...
Doxa génère les balises meta suivantes pour chaque page. Vous pouvez les remplacer en les définissant dans votre `docs.json` ou dans le frontmatter d&#39;une page.
...
* `generator: Doxa` - Identifie Doxa comme générateur du site
...
Doxa génère automatiquement une image Open Graph (OG) pour chaque page. Cette image apparaît comme aperçu social lorsque vous partagez un lien sur les réseaux sociaux et les applications de messagerie.
...
Si vous utilisez un domaine personnalisé, définissez la balise meta `canonical` dans votre `docs.json` pour vous assurer que les moteurs de recherche indexent votre domaine préféré. Doxa ajoute le chemin de chaque page à cette URL de base.
...
Doxa génère automatiquement un fichier `sitemap.xml` et un fichier `robots.txt`. Vous pouvez consulter votre sitemap en ajoutant `/sitemap.xml` à l'URL de votre site de documentation.

Par défaut, Doxa n'indexe que les pages incluses dans la navigation de votre `docs.json`. Il exclut les [pages masquées](/fr/organize/hidden-pages) qui existent dans votre dépôt mais ne figurent pas dans votre navigation de :
...
Ces valeurs par défaut aident les outils d'IA comme ChatGPT, Claude et Perplexity à découvrir et à citer votre documentation. Pour modifier les signaux, [ajoutez un `robots.txt` personnalisé](#custom-sitemaps-and-robotstxt-files) à la racine de votre projet. Les fichiers personnalisés sont servis tels quels et n'incluent pas les directives Content-Signal par défaut.
