...
This guide covers how to create different types of links in Doxa and how to maintain link integrity as your documentation grows.
...
Doxa resolves relative paths (`./` and `../`) based on the source file's location in your project directory. This works for links, images, and JSX elements like `<Card>` and `<a>` tags.
...
### How Doxa generates anchors

Doxa automatically creates anchors from header text by converting to lowercase, replacing spaces with hyphens, and removing special characters.
...
Run the Doxa CLI before publishing to catch broken internal and external links:
...
Root-relative paths (starting with `/`) are the most common choice for internal links in Doxa. They work consistently regardless of where the linking page sits in your directory, and they don't break if your documentation domain changes. Absolute URLs for internal links create unnecessary brittleness.
