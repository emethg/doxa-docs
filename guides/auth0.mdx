...
description: "Configure Auth0 as your OAuth provider for Doxa documentation authentication, including application setup, callback URLs, and token flow."
keywords: ["Auth0", "OAuth flow"]
---

<Note>
  **Security Disclaimer**: While we provide this guide to help you integrate Auth0 with Doxa, consult with your security team before implementing any authentication solution. Doxa is not responsible for any security issues that may arise from your specific implementation.
</Note>

## Overview

This guide walks you through setting up Auth0 as an authentication provider for your Doxa documentation. By the end, your users can log in to your documentation using their Auth0 credentials.

<Steps>
  <Step title="Create a Regular Web Application in Auth0">
    Log in to your Auth0 dashboard and navigate to **Applications** > **Applications**. Click **Create Application**, give your application a name (for example, "Doxa"), and select **Regular Web Applications** as the application type. Then click **Create**.
  ...
  <Step title="Get client information">
    ...
    You'll need these values for configuring Doxa in the next step.
  </Step>
  <Step title="Setup Doxa client">
    Navigate to your Doxa Dashboard and go to the **Settings** > **Authentication** section. Select **OAuth** as your authentication method and you'll see the OAuth configuration form:
    
    <Frame>![Doxa OAuth client settings](/images/guides/auth0/doxaoauthclientsettings.png)</Frame>
  ...
  <Step title="Configure Callback URL">
    Doxa generates a unique Redirect URL that Auth0 needs to recognize for the OAuth flow to work properly.
    
    Copy the Redirect URL from your Doxa Dashboard's Authentication settings:
    <Frame>![Doxa redirect URL](/images/guides/auth0/doxaredirecturl.png)</Frame>
