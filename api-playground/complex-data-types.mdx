...
<Warning>Doxa treats `oneOf` and `anyOf` identically since the practical difference rarely affects using the API.</Warning>
...
When you use `allOf`, Doxa performs some preprocessing on your OpenAPI document to display complex combinations in a readable way. For example, when you combine two object schemas with `allOf`, Doxa combines the properties of both into a single object. This becomes especially useful when you use OpenAPI's reusable [components](https://swagger.io/docs/specification/components/).
