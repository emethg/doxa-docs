...
<Note>
  The `directory` field is optional. If not specified, Doxa adds the files to the **api-reference** folder of the docs repository.
</Note>

### Examples with nested groups

The `asyncapi` property supports nested groups. Doxa generates the channel pages and adds them to the nested group, alongside any existing pages.
