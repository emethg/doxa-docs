...
description: "Install the Doxa CLI to preview documentation locally, test changes in real time, and catch build errors before deploying to production."
...
To create a new documentation project from the Doxa starter template, run the following command:
...
See available templates in the [doxa/templates](https://github.com/doxa/templates) repository on GitHub. In interactive mode, the CLI fetches and displays available templates automatically.
...
<AccordionGroup>
  ...
  <Accordion title="Issue: Encountering an unknown error">
    **Solution**: Go to the root of your device and delete the `~/.doxa` folder. Afterwards, run `mint dev` again.
  </Accordion>
  ...
  <Accordion title="doxa versus mint package">
    ...
    If you have both a `mint` and a `doxa` package installed, uninstall `doxa`:
  ...
  <Accordion title="Client version shows 'none' after installation">
    ...
    **Solution**: Ask your IT administrator to add `releases.doxa.com` to your network allowlist.
  </Accordion>
  <Accordion title="CLI connects to localhost instead of production when using npx">
    ...
    build. This made the CLI point to `localhost` URLs instead of the Doxa production
