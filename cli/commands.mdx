---
title: "Commands"
description: "Complete reference for every Doxa CLI command and flag, including mint dev, mint build, mint validate, mint analytics, and more."
...
Authenticate with your Doxa account.

```bash
mint login
```

Opens a browser window to complete authentication. If the browser does not open, the CLI displays a URL to open manually and a prompt to paste the authorization code. Credentials save in `~/.config/doxa/config.json`.
...
Manage persistent default values for CLI commands. The configuration saves in `~/.config/doxa/config.json`.
...
| `skillMd` | Agents can reach a [skill.md](https://www.doxa.com/docs/ai/skillmd) file for agent tool use. |
...
Create a new documentation project by picking a theme or cloning a pre-defined template from the [doxa/templates](https://github.com/doxa/templates) repository.
...
The CLI collects anonymous usage telemetry to help improve Doxa. Telemetry data includes the command name, CLI version, operating system, and architecture. Doxa does **not** collect personally identifiable information, project content, or file paths.
...
| --- | --- | --- |
| `DOXA_TELEMETRY_DISABLED` | `1` | Disable Doxa CLI telemetry. |
| `DO_NOT_TRACK` | `1` | Disable telemetry using the [Console Do Not Track](https://consoledonottrack.com/) standard. |

Your preference saves in `~/.config/doxa/config.json` and persists across CLI sessions.
