...
You must log in to use analytics commands. Run `mint login` to authenticate with your Doxa account.
...
To avoid repeating common flags, set defaults with `mint config`. The configuration saves in `~/.config/doxa/config.json`.
