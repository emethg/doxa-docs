---
title: "CI checks"
description: "Automate documentation quality checks in your CI/CD pipeline with broken link detection, linting, grammar validation, and build previews."
keywords: ["continuous integration", "CI/CD", "checks", "Vale", "linter"]
---

<Info>
  [Pro and Enterprise plans](https://doxa.com/pricing?ref=docs-ci) include CI checks for GitHub repositories.
</Info>

Use CI checks to lint your docs for errors and provide warnings before you deploy. Doxa CI checks run on pull requests against a configured deployment branch.
...
Configure the CI checks enabled for a deployment by navigating to the [Add-ons](https://app.doxa.com/products/addons) page of your dashboard. Enable the checks that you want to run.
...
To see detailed results for broken links on a pull request, click the **Checks** tab, then click the Doxa broken links check. The results list any files with broken links found in the pull request.
...
Doxa supports automatically running Vale in a CI check and displaying the results as a check status.
...
To add your own vocabulary for the default configuration, create a `styles/config/vocabularies/Doxa` directory with `accept.txt` and `reject.txt` files.
