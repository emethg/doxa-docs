...
Create a `.doxa/AGENTS.md` file at the root of your documentation project to customize the agent's behavior (`Agents.md` is also an accepted filename).

<Warning>
  Place your `AGENTS.md` file inside the `.doxa/` directory, not at the root of your project. If you store the file at the root of your project, your custom instructions are publicly accessible by the `/agents.md` path. The `.doxa/` directory is not served publicly so your `AGENTS.md` file is not accessible when in that directory.
