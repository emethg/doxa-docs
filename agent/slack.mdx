...
description: "Install the Doxa agent in Slack to ask questions about your product, create content updates from team conversations, and capture knowledge in pull requests."
keywords: ["Slack integration", "Slack bot", "team collaboration", "agent integration", "knowledge capture"]
---

<Note>
  If your Slack Workspace Owner requires admin approval to install apps, ask them to approve the Doxa app before you connect it.
</Note>
...
1. Go to the [Agent](https://app.doxa.com/products/agent) page in your dashboard.
...
3. Follow the Slack prompts to install the `doxa` app in your workspace and grant the requested permissions.
4. Confirm the connection in your Doxa dashboard to your Slack workspace.
5. Test that the agent is working and responds when you:
   - Send a direct message to the agent.
   - Mention the agent with `@doxa` in a channel.

<Note>
  Each Slack workspace can connect to only one Doxa organization at a time. If you try to install the agent into a workspace that is already connected to a different organization, the installation fails with a "wrong organization" error. Uninstall the app from the other organization first, or use a different Slack workspace.

  You can install the Slack agent even if you don't have a Doxa-hosted deployment. The agent still works for question answering and conversations. Features that open pull requests require a connected repository.
</Note>
...
<Warning>
  ...
  To add the app to a workspace, open your org's app management page at a URL like `https://app.slack.com/manage/<your-org-id>/integrations/installed`, find the `doxa` app, and install it to your workspace.
</Warning>

### Reconnect or reinstall the agent

If you must reauthorize the agent—for example, to grant new permissions after a Slack scope update or to recover from a revoked token—reinstall the app from your Doxa dashboard.

When you reinstall the agent into a workspace that is already connected to your organization, Doxa refreshes the stored bot token in place. Your existing channels, threads, and per-user integrations remain intact.
...
- Mention `@doxa` in a channel to use it publicly and collaboratively.
...
- **New question**: Send a direct message to the agent or mention `@doxa` in a channel with your question.
- **Existing thread**: Reply in the thread and mention `@doxa` with a follow-up question.
...
- **New request**: Send a direct message to the agent or mention `@doxa` in a channel with instructions on what to update.
- **Existing thread**: Reply in the thread and mention `@doxa` with instructions on what to update.
...
- **Viewers**: Can ask questions only. The agent runs in read-only mode for Doxa tools and third-party integrations.

To change a teammate's permissions, update their role on the [Members](https://app.doxa.com/settings/organization/members) page of your dashboard.
