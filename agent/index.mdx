...
description: "Learn how the Doxa agent researches, plans, and writes documentation, then opens pull requests with proposed changes for your team to review."
...
- **Validates**: Runs Doxa CLI checks to ensure documentation builds correctly.
...
- Reference source code from any repository that has the Doxa GitHub App installed.
...
By default, the agent opens pull requests attributed to the Doxa bot. To attribute pull requests to you, connect your GitHub account on the [My profile](https://app.doxa.com/settings/account) page of the dashboard.

## Connect repositories as context

The agent can only access repositories that you connect through the Doxa GitHub App. Configure which repositories the agent can access in the agent panel **Settings** or in the [GitHub App settings](https://github.com/apps/doxa/installations/new).

## Use AI tools alongside the agent

The agent works asynchronously through pull requests, but you can also use AI coding tools like Cursor or Claude Code locally for fast, iterative edits. Install the Doxa [skill](/ai/skillmd) and connect the [MCP server](/ai/model-context-protocol) so your editor has the same context the agent uses.

To let an external agent edit your content directly through MCP tool calls, connect it to the [Doxa MCP server](/ai/doxa-mcp).
