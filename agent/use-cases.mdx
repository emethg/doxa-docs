---
title: "Use cases"
description: "Explore real-world examples of using the Doxa agent to automate documentation updates, capture knowledge, and maintain accurate docs."
...
Prompt the agent, then continue to mention it with `@doxa` in the same thread to refine and iterate on the pull request that it creates.

For example: `@doxa Our quickstart page needs a new section on inviting collaborators`. Then `@doxa The new section should be called "Inviting collaborators"`. Followed by any other iterations.
...
For example: `@doxa Update the quickstart page to include information about inviting collaborators` and then checkout the branch to make any additional changes using your preferred method.
...
For example: `@doxa This PR adds a new authentication method. Update the docs to include the new auth flow: [PR link]`.
...
For example: `@doxa Generate release notes for this PR: [PR link]`.
...
For example: `@doxa Address the review comments on this PR: [PR link]`.

## Generate code examples

Prompt the agent to generate code examples for features throughout your docs or on specific pages.

For example: `@doxa Generate a code example to make the authentication method easier to understand`.
...
For example: `@doxa Add this diagram to the architecture overview page` with the image attached to the message.

## Review existing content

Prompt the agent to review existing content for technical accuracy, style, grammar, or other issues.

For example: `@doxa Review the API rate limiting section. We changed limits last month`.

## Respond to user feedback

Prompt the agent with feedback from your users to make focused updates to your docs.

For example: `@doxa Users are getting confused by step 3 in the setup guide. What might be making it unclear?`.
