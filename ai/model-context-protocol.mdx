...
The Model Context Protocol (MCP) is an open protocol that creates standardized connections between AI applications and external services, like documentation. Doxa generates a search MCP server for your site, preparing your content for the broader AI ecosystem. Any MCP client like Claude, Cursor, Goose, or ChatGPT can connect to your content.
...
<Tip>
  Looking to let agents edit your content instead of read it? Use the [admin MCP server](/ai/doxa-mcp) for an authenticated MCP server that exposes branching, page editing, navigation, and `docs.json` tools to trusted agents.
</Tip>

### How MCP servers work

When an AI application connects to your search MCP server, it can search your content and retrieve full pages in response to a user's prompt. This avoids relying on information from training data or generic web searches. Your search MCP server provides access to all indexed content on your Doxa site.
...
`Skill.md` resources appear in the search MCP server's resource list and contain the capability descriptions Doxa generates or that you define in your [custom skill files](/ai/skillmd#custom-skill-files).
...
Doxa generates a search MCP server for your site and hosts it at the `/mcp` path of your site URL. For example, Doxa's search MCP server is available at `https://doxa.com/docs/mcp`.
...
View and copy your search MCP server URL on the [MCP server page](https://app.doxa.com/products/mcp) in your dashboard.
...
Doxa hosts a discovery document at `/.well-known/mcp` for agents and tools to locate your search MCP server without prior configuration.
...
For agent-readiness, the same discovery document is also available at `/.well-known/mcp.json`. Doxa additionally serves an MCP server card at `/.well-known/mcp/server-card.json` and a list of server cards at `/.well-known/mcp/server-cards.json`. All discovery endpoints are served automatically and don't require configuration.
...
<Steps>
  <Step title="Enable authenticated MCP in your dashboard">
    1. Navigate to the [MCP server page](https://app.doxa.com/products/mcp) in your dashboard.
  ...
  <Step title="Add redirect domains">
    ...
    By default, redirect domains use `https://`. To allow a custom protocol scheme, include the full protocol in the domain entry. For example, a native app callback like `myapp://callback`. Doxa always blocks dangerous protocols such as `javascript:`, `data:`, and `file:`.
...
You can manage your client credentials from the [MCP server page](https://app.doxa.com/products/mcp) in your dashboard.
...
To protect availability, Doxa applies rate limits to MCP servers.
...
Your MCP server searches content that Doxa indexes from your project repository. File processing and search indexing control what content is available through your MCP server.
...
If files match [.mintignore](/organize/mintignore) patterns, Doxa does not process or index them. These files are not available through your MCP server.

### Search indexing with `docs.json`

By default, Doxa only indexes pages included in your `docs.json` navigation for search through your MCP server.

Doxa excludes [hidden pages](/organize/hidden-pages) (pages not in your navigation) from the search index unless you choose to index all pages. To include hidden pages in your MCP server's search results, add the `seo.indexing` property to your `docs.json`.
...
<Steps>
  <Step title="Get your MCP server URL.">
    Navigate to your [dashboard](https://app.doxa.com/products/mcp) and find your MCP server URL.
  </Step>
...
### Example: Connect to the Doxa MCP server

Connect to the Doxa MCP server to search this documentation site within your preferred AI tool. This gives you more accurate answers about how to use Doxa in your local environment and demonstrates how you can help your users connect to your MCP server.

<Tabs>
<Tab title="Contextual menu">
  At the top of this page, click the contextual menu, then click **Connect to Cursor** or **Connect to VS Code** to connect the Doxa MCP server to the IDE of your choice.
</Tab>
<Tab title="Claude">

To use the Doxa MCP server with Claude:

<Steps>
  <Step title="Add the Doxa MCP server to Claude">
    1. Navigate to the [Connectors](https://claude.ai/settings/connectors) page in the Claude settings. 
    2. Click **Add custom connector**.
    3. Add the Doxa MCP server:
      - Name: `Doxa`
  ...
  <Step title="Access the MCP server in your chat">
    ...
    2. Select the Doxa MCP server.
    3. Ask Claude a question about Doxa.
  </Step>
</Steps>
...
To use the Doxa MCP server with Claude Code, run the following command:
...
To connect the Doxa MCP server to Cursor, click the **Install in Cursor** button. Or to manually connect the MCP server, follow these steps:
...
<Steps>
  ...
  <Step title="Test the connection">
    In Cursor's chat, ask "What tools do you have available?" Cursor should show the Doxa MCP server as an available tool.
  </Step>
</Steps>
...
To connect the Doxa MCP server to VS Code, click the **Install in VS Code** button. Or to manually connect the MCP server, create a `.vscode/mcp.json` file and add:
