...
Doxa automatically handles many SEO best practices, including:
...
Doxa generates the following meta tags for every page. You can override these meta tags by specifying them in your `docs.json` or a page's frontmatter.
...
- `generator: Doxa` - Identifies the site generator as Doxa
...
Doxa automatically generates an Open Graph (OG) image for every page. This image appears as the social preview when you share a link on social media platforms and messaging apps.
...
If you're using a custom domain, set the `canonical` meta tag in your `docs.json` to ensure search engines index your preferred domain. Doxa appends each page's path to this base URL.
...
<Tip>
  Setting `og:image` in meta tags replaces the auto-generated social preview with a static image. If you want a custom **background image** that Doxa automatically overlays with your logo, page title, and description, use [`thumbnails.background`](/organize/settings-reference#thumbnails-background) in your `docs.json` instead.
</Tip>
...
Doxa automatically generates a `sitemap.xml` file and a `robots.txt` file. You can view your sitemap by appending `/sitemap.xml` to your documentation site's URL.

By default, Doxa indexes only the pages you include in your `docs.json` navigation. It excludes [hidden pages](/organize/hidden-pages) that exist in your repository but are not listed in your navigation from:
