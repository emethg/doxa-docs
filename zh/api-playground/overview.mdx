...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="proxy" type="boolean" defaultOpen="True">
      是否通过 Doxa 的代理服务器转发 API 请求。默认为 `true`。

      当设置为 `true` 时，playground 请求会通过 Doxa 的服务器路由。当设置为 `false` 时，playground 会直接从浏览器向你的 API 发送请求。当你的 API 接受浏览器直接请求且不需要 Doxa 代理流量时，请设置为 `false`。例如，当你的 API 需要无法通过代理转发的特定请求头时，或者当你需要请求直接从用户浏览器发起以进行身份验证时。
    </ResponseField>
