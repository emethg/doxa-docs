...
1. 在控制台前往 [Custom domain setup](https://dashboard.doxa.com/settings/deployment/custom-domain) 页面。
...
<Warning>
  ...
  如果你正在迁移现有域名并希望实现零停机，请先发布验证 `TXT` 记录，待其显示为已验证且 TLS 完成预签发后，再将 `CNAME` 指向 Doxa。
</Warning>
...
1. 进入控制台中的[自定义域名设置](https://dashboard.doxa.com/settings/deployment/custom-domain)页面。
2. 找到处于待验证状态的自定义域名。
...
将 `https://www.your-custom-domain-here.com` 替换为你的实际自定义 domain。举例来说，如果你的自定义 domain 是 `docs.doxa.com`，你应使用：
