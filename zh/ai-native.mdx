...
title: "AI 原生文档"
sidebarTitle: "AI 原生"
description: "了解 AI 原生功能如何通过助手、agent 和 MCP 服务器提升文档的阅读、创作和发现体验。"
keywords: ["AI","assistant","agent","llms.txt","MCP","llms-full.txt"]
---

当你在 Doxa 上托管文档时，内置的 AI 功能可帮助用户更快找到答案，并让你的团队更高效地维护内容。你的内容为这些 AI 原生功能提供了上下文，从而全面提升阅读、创作与检索文档的体验。
...
创建 [workflows](/workflows/index)，让 agent 按预设计划或在推送到存储库时运行。每个 workflow 都为 agent 定义提示词以及触发运行的条件。

配置常用工具 (如 [Cursor](/zh/guides/cursor)、[Claude Code](/zh/guides/claude-code) 和 [Windsurf](/zh/guides/windsurf)) ，以对齐 Doxa 架构、你的风格指南和最佳实践。
...
Doxa 会为你的文档托管 `llms.txt` 和 `skill.md` 文件。这些行业标准文件帮助大语言模型在响应用户查询时高效返回相关信息，并为智能代理提供可用能力列表，从而让用户在使用你的产品时取得更好的效果。
