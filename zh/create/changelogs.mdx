---
title: "更新日志"
description: "创建包含基于日期条目、RSS 订阅支持和订阅者通知功能的产品更新日志，让用户及时了解最新动态。"
...
查看 [Doxa 更新日志](/zh/changelog) 作为示例：你可以在每次更新中包含指向新功能的链接、图片、文本和演示。
...
当用户选择多个标签时，更新日志仅显示包含所有所选标签的更新。只要存在已激活的筛选器，没有标签的更新就会被隐藏。
...
使用 `Update` 组件会在你的页面 URL 后追加 `/rss.xml`，从而创建一个可订阅的 RSS 源。例如：`doxa.com/docs/changelog/rss.xml`。
