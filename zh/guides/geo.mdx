...
默认情况下，Doxa 会索引包含在 `docs.json` 导航中的页面。要将隐藏页面包含在 AI 助手上下文和搜索中：
...
Doxa 会自动为你的文档生成 `llms.txt` 文件。LLMs.txt 的工作方式类似于传统搜索中的 `sitemap.xml`——它为 AI 系统提供文档的结构化索引。无需任何配置。
...
Doxa 自动生成的 `robots.txt` 默认允许所有爬虫，并包含 [Content-Signal 指令](/zh/optimize/seo#content-signal-directives)，可让 AI 工具将你的文档用于模型训练、搜索索引和 AI 回答生成。如果你使用[自定义 robots.txt 文件](/zh/optimize/seo#custom-sitemaps-and-robotstxt-files)，请确保它不会屏蔽 AI 代理。最常见的 AI 用户代理包括：
...
LLMs.txt 是一种约定——类似于 robots.txt——用于为 AI 系统提供文档的结构化索引。Doxa 会自动生成它。你不需要配置它。你可以在 `https://your-docs-domain.com/llms.txt` 查看你的 LLMs.txt 文件。
</Accordion>
</AccordionGroup>
