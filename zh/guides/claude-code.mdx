...
description: "通过 CLAUDE.md 项目说明配置 Claude Code，按照你的风格指南撰写、审阅和更新你的 Doxa 文档。"
...
- 在构建 docs.json 文件和站点导航时，请参考 [docs.json 架构](https://doxa.com/docs.json)
...
<AccordionGroup>
<Accordion title="使用 Claude Code 与 Doxa 是否需要 CLAUDE.md 文件？">
...
<Accordion title="Claude Code 与 Cursor 或 Windsurf 在文档编写方面有什么区别？">
Claude Code 是一款命令行工具，专为在整个代码仓库范围内执行智能体式、多步骤任务而设计。它非常适合审查所有页面是否缺少替代文本、在每个代码示例中更新参数名称，或检查一组文件的一致性等任务。Cursor 和 Windsurf 是基于 IDE 的工具，更适合通过内联建议编辑单个文件。两种方式都可以——正确的选择取决于你的工作是逐个文件还是跨整个仓库。
</Accordion>

<Accordion title="我能否使用 Claude Code 从代码自动生成文档？">
