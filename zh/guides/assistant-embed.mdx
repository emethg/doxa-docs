...
description: "构建并嵌入一个应用内文档助手，使用来自你的 Doxa 文档站点的引用信息回答用户问题。"
...
* [Doxa Pro 或 Enterprise 计划](https://doxa.com/pricing)
* 你的 domain 名称，它位于控制台 URL 的末尾。例如，如果你的控制台 URL 是 `https://dashboard.doxa.com/org-name/domain-name`，你的 domain 名称就是 `domain-name`
* 一个 [AI 助手 API key](https://dashboard.doxa.com/settings/organization/api-keys)
...
1. 在控制台中前往 [API keys](https://dashboard.doxa.com/settings/organization/api-keys) 页面。
...
克隆[示例存储库](https://github.com/doxa/assistant-embed-example)，并按需进行自定义。
...
<Steps>
  ...
  <Step title="配置你的项目">
    打开 `src/config.js`，并填入你的 Doxa 项目信息。
    ...
    * 将 `your-domain` 替换为你在控制台 URL 末尾看到的 Doxa 项目 domain。
    * 将 `https://yourdocs.doxa.app` 替换为你的文档实际 URL。
  </Step>
