...
description: "使用工作区规则和记忆配置 Windsurf Cascade，以编写遵循你的样式指南和 MDX 标准的 Doxa 文档。"
keywords: ["Windsurf", "Cascade", "workspace rules", "AI 文档", "Windsurf 文档"]
---

通过 workspace rules 和 memories，将 Windsurf 打造成一位熟悉你的样式指南、组件和项目 context 的文档专家。

<div id="use-windsurf-with-doxa">
  ## 将 Windsurf 与 Doxa 配合使用
</div>

Windsurf 的 Cascade AI 助手可以配合 Doxa 组件，按照你的标准撰写文档。工作区规则与记忆会为你的项目提供持久的 context，从而让 Cascade 的建议更加一致。
...
此规则为 Cascade 提供有关 Doxa 组件和通用技术写作最佳实践的 context。
...
- 在构建 docs.json 文件和网站导航时，请参考 [docs.json 架构](https://doxa.com/docs.json)
...
将 Doxa MCP 服务器连接到 Windsurf，使 Cascade 在帮助你写作时可以搜索 Doxa 文档。连接 MCP 服务器后，Cascade 会在最新的 Doxa 文档中搜索相关上下文，这样你就不需要离开 IDE 来查阅文档。

完整的设置说明请参见 [Model Context Protocol](/zh/ai/model-context-protocol#example%3A-connect-to-the-doxa-mcp-server)。