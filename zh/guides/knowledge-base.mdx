...
title: "创建知识库"
sidebarTitle: "知识库"
description: "在 Doxa 上托管团队内部知识库，以整合信息、提升搜索效果，并降低维护成本。"
...
Doxa 提供搭建知识库的基础设施，让整个团队都能参与贡献。

* **AI 驱动搜索**：通过 [AI 助手](/zh/assistant/index) 使用你的知识库内容来回答问题，让用户即使不知道去哪找，也能找到答案。
...
如果你尚未创建 Doxa 项目，请参阅 [快速开始](/zh/quickstart) 来部署你的站点。
...
* 导出为 **Markdown**，这是迁移到 Doxa 的最简方式。 (推荐) 
...
<Steps>
  <Step title="整理文件">
    ...
    将每篇文章放入其对应的文件夹中。路径必须与 `docs.json` 中的路径一致。例如，如果 `docs.json` 中引用的是 `"finance/expense-reports"`，则项目存储库中的文件应为 `finance/expense-reports.mdx`。
  </Step>
...
在[控制台](https://dashboard.doxa.com/products/assistant/settings)中配置 AI 助手：
...
只有当不只是最初搭建的人，而是所有人都能参与更新时，知识库才能保持准确。Doxa 为团队成员提供三种方式，帮助他们快速参与维护你的知识库。
...
当有人在 Slack 线程中分享了有用信息时，任何团队成员都可以通过提及 `@doxa`，并附上让其整理成文档的指令来触发操作。智能代理会读取对话内容，提取相关信息，并创建一个拉取请求 (PR；亦称“合并请求”/Merge Request) 。这对于在上下文仍然新鲜时捕获技术决策、故障排除方案以及流程说明非常有用。

例如，如果你的团队在某个线程中讨论了如何配置一个新的集成，你可以回复：

> @doxa 根据这段对话创建一篇关于配置 Acme 集成的指南。
