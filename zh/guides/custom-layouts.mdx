...
description: "使用页面模式和组件在 Doxa 文档中构建自定义落地页、营销页面及其他非标准布局。"
keywords: ["landing page", "page mode", "custom mode", "frame mode", "hero section", "landing pages"]
---

Doxa 页面默认使用标准布局，包含侧边栏、内容区域和目录。你可以通过[页面模式](/zh/organize/pages#page-mode)自定义此布局，以创建落地页、功能展示页或任何需要独特设计的页面。
...
对于 Tailwind 未涵盖的样式，可向你的仓库中添加 CSS 文件。Doxa 会在全站范围内应用 CSS 文件，使其类名在所有 MDX 文件中可用。
