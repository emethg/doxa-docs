---
title: "全局设置"
description: "使用 docs.json 配置你的 Doxa 文档站点，该文件控制导航、外观和集成。"
keywords: ["docs.json", "配置", "设置", "mint.json"]
---

`docs.json` 文件是 Doxa 文档站点的核心配置文件。它控制站点的全局设置，包括视觉品牌、导航结构、集成、API 设置等。可以把它看作站点的蓝图。
...
在 `docs.json` 中的任何位置添加一个带有相对文件路径的 `$ref` 属性。Doxa 会将 `$ref` 对象替换为被引用文件的内容。
...
如果 `$ref` 解析为对象，Doxa 会将同一块中的兄弟键合并到引用内容之上，使这些键优先于引用中的匹配键。如果 `$ref` 解析为非对象值（如数组），Doxa 会忽略任何兄弟键。
