...
<Note>
  当你在某个导航元素（例如 anchor、tab 或 group）上添加 `openapi` 属性且未指定任何页面时，Doxa 会自动为 OpenAPI 规范中定义的 **所有端点** 生成页面。
...
Doxa 使用 `versions` 数组中的第一个版本作为默认版本。使用 `default` 字段可将其他版本指定为默认版本。
...
如需自动翻译，[设置工作流](/workflows)，让代理按计划运行或在仓库推送时触发。
