---
title: "命令参考"
description: "Doxa CLI 所有命令和选项的完整参考，包括 mint dev、mint build、mint validate、mint analytics 等。"
...
打开浏览器窗口完成身份验证。如果浏览器未打开，CLI 会显示一个 URL 供你手动打开，并提示你粘贴授权代码。凭据保存在 `~/.config/doxa/config.json` 中。
...
| 选项 | 描述 |
| --- | --- |
| `--subdomain` | 文档子域名。默认使用 `mint config set subdomain` 设置的值，或你账户中的第一个项目。 |
...
管理 CLI 命令的持久默认值。配置保存在 `~/.config/doxa/config.json` 中。
...
| `skillMd` | 代理可以访问 [skill.md](https://www.doxa.com/docs/ai/skillmd) 文件以供代理工具使用。 |
...
通过选择主题或从 [doxa/templates](https://github.com/doxa/templates) 仓库克隆预定义模板来创建新的文档项目。
...
CLI 收集匿名使用遥测数据以帮助改进 Doxa。遥测数据包括命令名称、CLI 版本、操作系统和架构。Doxa **不会**收集个人身份信息、项目内容或文件路径。
...
| 变量 | 值 | 描述 |
| --- | --- | --- |
| `DOXA_TELEMETRY_DISABLED` | `1` | 禁用 Doxa CLI 遥测。 |
| `DO_NOT_TRACK` | `1` | 使用 [Console Do Not Track](https://consoledonottrack.com/) 标准禁用遥测。 |

你的偏好保存在 `~/.config/doxa/config.json` 中，在 CLI 会话之间持久有效。
