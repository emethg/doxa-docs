---
title: "本地预览"
description: "使用 mint dev 运行 Doxa 文档站点的本地预览，支持实时重载、全文搜索和 AI 助手。"
...
你需要使用 Doxa 账户对 CLI 进行身份验证，才能启用搜索和 [AI 助手](/zh/assistant/index)。

```bash
mint login
```

使用 `mint login` 命令完成身份验证后，CLI 会将你的凭据存储在 `~/.config/doxa/config.json` 中，以便你在各会话之间保持登录状态。CLI 还会提示你选择一个默认项目，`mint analytics` 等命令会使用该项目。
