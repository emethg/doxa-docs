---
title: "Osano"
description: "将 Osano 集成到你的 Doxa 文档站点，管理 Cookie 同意横幅、隐私偏好设置和监管合规要求。"
...
<Accordion title="在 Strict 合规模式下页面无法加载">
  如果在使用 Osano 的 **Strict** 合规模式时文档页面无法正常加载，你需要将 Doxa 的 domain 加入允许列表，以允许图片和其他资源加载。
  ...
  <Steps>
    ...
    <Step title="添加 Doxa domain">
      将 `.doxa.app/` 添加为一条 managed rule。
      ...
      <Info>
        这可确保所有由 Doxa 提供的资产 (包括图片、样式表和其他文档资源) 被视为必要内容，即使 Osano 阻止未分类的第三方内容时也会加载。
      </Info>
    </Step>
  </Steps>
</Accordion>