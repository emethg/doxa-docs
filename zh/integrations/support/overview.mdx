...
将客户支持小部件集成到你的文档中。向你的 `docs.json` 文件添加 `integrations` 字段，并填入对应的应用 ID 或聊天 ID。查阅各个集成的文档以了解如何找到你的 ID。