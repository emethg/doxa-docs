...
<Info>
  SSO 功能适用于 [企业套餐](https://doxa.com/pricing?ref=sso)。
</Info>

企业管理员可以直接在 Doxa 控制台中为 Okta 或 Microsoft Entra 配置基于 SAML 的 SSO。对于 Google Workspace 或 Okta OIDC 等其他提供商，请[联系我们](mailto:support@doxa.com)，以设置 SSO。
...
<Steps>
  <Step title="在 Doxa 控制台中配置 Okta SSO">
    1. 在 Doxa 控制台中，前往 [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso) 页面。
    2. 点击 **Configure**。
    3. 选择 **Okta SAML**。
  ...
  <Step title="在 Okta 中创建 SAML 应用">
    ...
    2. 输入来自 Doxa 的以下信息：
        * **Single sign on URL**：你从 Doxa 控制台复制的 URL
        * **Audience URI**：你从 Doxa 控制台复制的 URI
  ...
  <Step title="在 Doxa 中保存">
    回到 Doxa 控制台，粘贴 metadata URL，然后点击 **保存更改**。
  </Step>
</Steps>
...
<Steps>
  <Step title="在 Doxa 控制台中配置 Microsoft Entra SSO">
    1. 在 Doxa 控制台中，导航到 [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso) 页面。
    2. 点击 **Configure**。
  ...
  <Step title="在 Microsoft Entra 中配置 SAML">
    ...
    3. 在 **Basic SAML Configuration** 下，输入以下内容：
        * **Identifier (Entity ID)**：来自 Doxa 的 Audience URI
        * **Reply URL (Assertion Consumer Service URL)**：来自 Doxa 的 Single sign on URL
  ...
  <Step title="在 Doxa 中保存">
    返回 Doxa 控制台，粘贴 metadata URL 并点击 **保存更改**。
  </Step>
  <Step title="分配用户">
    在 Microsoft Entra 中，导航到 **Users and groups**，为需要访问 Doxa 控制台的用户进行分配。
  </Step>
</Steps>
...
当你启用 JIT（Just-in-time，即时）预配时，通过你的身份提供商登录的用户会被自动添加到你的 Doxa 组织中。

<Note>
  JIT 预配仅适用于由 IdP 发起的登录。用户必须从你的身份提供商（Okta 控制台或 Microsoft Entra 门户）发起登录，而不是从 Doxa 登录页面开始。
</Note>

要启用 JIT 预配，你必须先启用 SSO。前往控制台中的 [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso) 页面，完成 SSO 设置，然后启用 JIT 预配。

<div id="map-rbac-roles-with-saml-groups">
  ## 将 RBAC 角色映射到 SAML 组
</div>

根据用户在身份提供商中的组成员身份分配[角色](/zh/dashboard/roles)。当用户通过 SSO 登录时，Doxa 会读取 SAML 断言中的 `groups` 属性，并将这些组映射到控制台角色。
...
配置完成后，Doxa 会将 SAML 断言中的组名称映射到你组织中的角色。要设置或修改组到角色的映射，请联系你的 Doxa 客户代表。

<div id="change-or-remove-sso-provider">
  ## 更改或移除 SSO 提供商
</div>

1. 在控制台中前往 [Single Sign-On](https://dashboard.doxa.com/settings/organization/sso) 页面。
2. 点击 **Configure**。
...
如果你使用 Microsoft Entra 或 Okta SAML 之外的其他提供商，请[联系我们](mailto:support@doxa.com)以配置 SSO。
...
<Steps>
  ...
  <Step title="向我们提供 IdP 信息">
    复制提供的 SSO URL、Entity ID 和 x509 证书，并发送给 Doxa 团队。
...
<Steps>
  ...
  <Step title="配置集成">
    选择授权码（authorization code）授权类型，并输入由 Doxa 提供的重定向 URI（Redirect URI）。
  </Step>
