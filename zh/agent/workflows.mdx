...
1. 打开 [Workflows](https://dashboard.doxa.com/products/workflows) 页面。
2. 点击 **New workflow**。
...
<Frame>
<img src="/images/workflows/workflow-settings-light.png" alt="新建工作流的设置页面。" className="block dark:hidden" />
<img src="/images/workflows/workflow-settings-dark.png" alt="新建工作流的设置页面。" className="hidden dark:block" />
</Frame>
...
对于 GitHub 存储库，你必须在上下文或触发器字段中列出的每个存储库上安装 Doxa GitHub App。请在 Doxa 控制台的 [GitHub app](https://dashboard.doxa.com/settings/organization/github-app) 页面中添加新的存储库。

对于 GitLab 存储库，你必须在 Doxa 控制台的 [GitLab OAuth](https://dashboard.doxa.com/settings/organization/gitlab-oauth) 页面上连接你的 GitLab 账户。然后添加你想要与工作流一起使用的存储库。你必须在每个项目上至少拥有 Maintainer 角色才能连接 GitLab OAuth。
...
<Note>
  对于 GitHub 存储库，自动合并要求 Doxa GitHub 应用对所有针对你部署分支的规则集都拥有绕过权限，包括组织级和仓库级的规则集。请参阅[配置自动合并](/zh/guides/configure-automerge)了解设置说明。

  对于 GitLab 存储库，自动合并使用在 Doxa 控制台的 [GitLab OAuth](https://dashboard.doxa.com/settings/organization/gitlab-oauth) 页面上配置的 GitLab OAuth 连接。你必须在每个项目上至少拥有 Maintainer 角色才能连接 GitLab OAuth。
</Note>
...
<Note>
Slack 通知要求你的组织的 Slack 工作区中已安装 Doxa Slack 应用。请从你的[控制台](https://dashboard.doxa.com/products/agent)安装 Slack 应用。
</Note>
...
从控制台的 [Workflows](https://dashboard.doxa.com/products/workflows) 页面编辑、删除、禁用或触发工作流。
...
1. 前往控制台中的 [Workflows](https://dashboard.doxa.com/products/workflows) 页面。
...
当你重新启用基于 cron 的工作流时，Doxa 会从当前时间重新计算下次运行时间。
