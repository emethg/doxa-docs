---
title: "使用场景"
description: "探索使用 Doxa agent 自动更新文档、沉淀知识和维护准确文档的实际案例。"
...
先向 agent 发送提示词，然后在同一条线程中继续使用 `@doxa` 提及它，以便对其创建的拉取请求 (PR；亦称“合并请求”/Merge Request) 进行细化和迭代。

例如：`@doxa Our quickstart page needs a new section on inviting collaborators`。接着：`@doxa The new section should be called "Inviting collaborators"`。然后再根据需要继续进行其他迭代。
...
例如：`@doxa Update the quickstart page to include information about inviting collaborators`，然后检出该 branch，使用你喜欢的方式进行进一步修改。
...
例如：`@doxa This PR adds a new authentication method. Update the docs to include the new auth flow: [PR link]`.
...
例如：`@doxa Generate release notes for this PR: [PR link]`。
...
例如：`@doxa Address the review comments on this PR: [PR link]`。
...
例如：`@doxa Generate a code example to make the authentication method easier to understand`.
...
例如：`@doxa Add this diagram to the architecture overview page`，并在消息中附上该图片。
...
例如：`@doxa 请审查 API 限流部分。我们上个月更改了限制。`
...
例如：`@doxa Users are getting confused by step 3 in the setup guide. What might be making it unclear?`.
