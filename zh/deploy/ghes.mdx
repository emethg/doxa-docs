...
description: "在你的 GitHub Enterprise Server 实例上安装和配置 Doxa GitHub 应用，实现文档自动部署和同步。"
keywords: ["GitHub Enterprise Server", "GHES", "GitHub Enterprise"]
---

本指南将逐步引导你在 GitHub Enterprise Server（GHES）实例上配置 Doxa GitHub App。要将 GHES 实例连接到 Doxa，你必须在自托管环境中创建该应用的本地版本，用于与我们的远程服务器通信。
...
* Doxa 的 API 端点 (https://leaves.doxa.com)
* Webhook 接收端（443 端口）
...
* 来自 Doxa 静态 IP 地址的连接：`54.242.90.151`
* 到 Doxa 服务域名的 HTTPS（端口 443）连接
* 对 Doxa 服务域名的 DNS 解析
...
<Steps>
  ...
  <Step title="设置 webhook URL">
    ...
    * **Webhook URL：** `https://leaves.doxa.com/github-enterprise/:subdomain`（将 `:subdomain` 替换为我们提供给你的 URL）
    * **Webhook secret：** 生成一个随机字符串（32 个以上字符）并妥善保存。Doxa 也可以为你生成并提供此密钥。
  </Step>
</Steps>
...
* 将你的 GHES 环境与现有的 Doxa 部署集成。
* 将你的 GHES 环境与我们为你准备的新 Doxa 部署集成。

当你的 GHES 环境与某个 Doxa 部署完成集成后，你就可以为 GitHub App 启用 webhook 了。
...
<AccordionGroup>
  ...
  <Accordion title="如果我需要帮助，应联系谁？">
    请联系你在 Doxa 对接的客户成功代表，或通过 <a href="mailto:support@doxa.com">support@doxa.com</a> 联系我们的支持团队，并提供：
