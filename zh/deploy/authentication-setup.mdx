...
<Info>
  [Pro 方案](https://doxa.com/pricing?ref=authentication) 包含密码认证。

  [Enterprise 方案](https://doxa.com/pricing?ref=authentication) 包含所有认证方式。
</Info>

<Warning>
  认证仅适用于托管在自定义域名或 Doxa 子域名上的文档（例如 `docs.example.com` 或 `example.doxa.dev`）。使用[自定义基础路径](/zh/deploy/docs-subpath)（例如 `example.com/docs`）时**不支持**认证功能。
</Warning>
...
<Tabs>
  <Tab title="密码">
    ...
    * 你的安全策略允许在多个用户之间共享密码。
    ...
    <Steps>
      <Step title="创建密码。">
        1. 在控制台中，前往 [Authentication](https://dashboard.doxa.com/products/authentication)。
        2. 启用认证。
  ...
  <Tab title="Doxa 控制台">
    ...
    * 所有需要访问你文档的人都必须是你 Doxa 组织的成员。

    ### Doxa 控制台设置

    <Steps>
      <Step title="在 Doxa 控制台中启用认证。">
        1. 在控制台中，前往 [Authentication](https://dashboard.doxa.com/products/authentication)。
        2. 启用认证。
        3. 在 **Custom Authentication** 部分，点击 **Doxa Auth**。
        4. 点击 **Enable Doxa Auth**。

        启用 Doxa 认证后，你的网站会重新部署。部署完成后，任何访问你网站的人都必须登录到你的 Doxa 组织才能访问你的内容。
      </Step>

      <Step title="添加授权用户。">
        1. 在控制台中，前往 [Members](https://dashboard.doxa.com/settings/organization/members)。
        2. 添加所有需要访问你文档的人员。
        3. 根据他们的编辑权限分配合适的角色。
      </Step>
    </Steps>
    ...
    * 能够创建可通过 OAuth 访问令牌访问的 API 端点 (可选，用于启用基于用户组的访问控制) 。
    ...
    <Steps>
      <Step title="配置你的 OAuth 设置。">
        1. 在控制台中前往 [Authentication](https://dashboard.doxa.com/products/authentication)。
        2. 启用认证。
        ...
        * **Info API URL** (可选) ：你服务器上的一个端点，Doxa 会调用它来获取用户信息。对于基于用户组的访问控制是必填项。如果省略，OAuth 流程只会验证身份。
        * **Logout URL** (可选) ：你的 OAuth 提供方自带的登出 URL。用户登出时，Doxa 会将登出重定向与该配置的 URL 进行校验，以确保安全性。只有当重定向地址与配置的 `logoutUrl` 完全匹配时，重定向才会成功。如果你未配置登出 URL，用户会被重定向到 `/login`。Doxa 会使用 `GET` 请求重定向用户，并且不会追加任何 query 参数，因此请将所有参数 (例如 `returnTo`) 直接包含在 URL 中。
      ...
      <Step title="配置你的 OAuth 服务器。">
        1. 从你的[认证设置](https://dashboard.doxa.com/products/authentication)中复制 **Redirect URL**。
      ...
      <Step title="创建用户信息端点（可选）。">
        ...
        * 以 `User` 格式返回用户数据。更多信息参见 [User data format](#user-data-format)。
        ...
        Doxa 使用 OAuth 访问令牌调用此端点以获取用户信息。不会发送额外的 query 参数。

        将此端点 URL 填入你[认证设置](https://dashboard.doxa.com/products/authentication)中的 **Info API URL** 字段。
      </Step>
    </Steps>
  ...
  <Tab title="JWT（JSON Web Token）">
    ...
    * 一个可以创建重定向 URL 的后端服务。
    ...
    <Steps>
      ...
      <Step title="将 Doxa 认证集成到你的登录流程中。">
...
<ParamField path="host" type="string">
  **JWT 认证时必填。** 你的文档站点的主机名。该字符串必须与你部署文档的 domain 完全一致。Doxa 会验证 JWT 的 host 是否与发起请求的 host 匹配，以防止令牌在不同站点之间被重复使用。
</ParamField>
...
启用认证后，部分功能的行为会有所不同，或可能不可用。Doxa 不支持在需要认证的站点上公开提供任意文件。所有托管文件，包括 `llms.txt`、`llms-full.txt` 和 `skill.md`，都受与你的文档页面相同的认证要求约束。
