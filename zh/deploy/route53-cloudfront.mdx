...
sidebarTitle: "AWS"
description: "使用 AWS Route 53 进行 DNS 路由，并通过 CloudFront CDN 和 Lambda@Edge 函数将你的 Doxa 文档部署到子路径。"
...
3. 在 Origin domain 中输入 `[SUBDOMAIN].doxa.dev`，其中 `[SUBDOMAIN]` 是你项目的唯一子域。

<Frame>
    ![CloudFront “Create distribution” 页面显示 “acme.doxa.dev” 作为 Origin domain。](/images/cloudfront/origin-name.png)
  </Frame>
...
2. 找到与你主域名对应的预发布环境 URL。具体取决于你的落地页托管方式。例如，Doxa 的预发布 URL 是 [doxa-landing-page.vercel.app](https://doxa-landing-page.vercel.app)。
...
此时，你应当有两个 Origins：一个为 `[SUBDOMAIN].doxa.app`，另一个为你的预发布 URL。

<Frame>
  ![CloudFront 的 “Origins” 页面，包含两个 origins：一个用于 doxa，另一个用于 doxa-landing-page。](/images/cloudfront/final-origins.png)
</Frame>
...
- **如果用户访问你的自定义子路径**，跳转到 `[SUBDOMAIN].doxa.dev`。
...
创建一个行为，将 **Path pattern** 设置为你选择的子路径，例如 `/docs`，并将 **Origin and origin groups** 指向 `.doxa.dev` 的 URL（在我们的示例中为 `acme.doxa.dev`）。
...
<Frame>
  ![CloudFront 的 "Create behavior" 页面，其中 "Path pattern" 设置为 "/docs/*"，并且 "Origin and origin groups" 指向 `acme.doxa.dev` 的 URL。](/images/cloudfront/behavior-1.png)
</Frame>
...
创建一个行为，在 **Path pattern** 中填写你选择的子路径并在后面添加 `/*`，例如 `/docs/*`，并将 **Origin and origin groups** 指向相同的 `.doxa.dev` URL。
...
1. 将默认行为的 **Origin and origin groups** 更改为预发布环境的 URL（在我们的示例中为 `doxa-landing-page.vercel.app`）。
...
所有页面应当指向你的主着陆页；但如果在该 URL 后追加你选择的子路径（例如 `/docs`），则应会跳转到你的 Doxa 文档实例。
