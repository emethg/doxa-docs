...
description: "排查通过 Cloudflare Workers 提供 Doxa 文档时被拦截的请求并解决 Cloudflare WAF 规则冲突。"
keywords: ["Cloudflare WAF", "firewall rules", "Bot Fight Mode", "403 errors"]
---

如果你的文档站点在运行几秒后出现 500 错误，或导航变慢，可能是 Cloudflare 防火墙拦截了对 Doxa 资源的请求。
...
- 页面间的客户端导航缓慢或异常
- 对 `/doxa-assets/*` 路径的请求在浏览器控制台中显示 403 错误
...
由于以下原因，Cloudflare 的 Web Application Firewall（WAF）和 Bot Fight Mode 可能会将 Doxa 的资源请求判定为可疑：
...
创建一条 Cloudflare 防火墙规则，将 Doxa 资产排除在安全检查之外。
...
- 将 `contains` 运算符用于 `/doxa-assets/*` - `*` 会被视为普通字符，而非通配符
- 对 URI Path 使用 `equals` - 这只会匹配精确路径 `/doxa-assets/`，不匹配子路径
