...
<ResponseField name="navbar.links" type="array of object">
  ...
  <Expandable title="navbar.links">
    ...
    <ResponseField name="label" type="string">
      Link text. Required when `type` is not set. Optional for `github` and `discord`. If omitted, Doxa generates the label from API data.
    </ResponseField>
    <ResponseField name="href" type="string (uri)" required>
...
Global variables for use throughout your documentation. Doxa replaces `{{variableName}}` placeholders with the defined values at build time.
...
<ResponseField name="variables.[variableName]" type="string">
  ...
  - Doxa sanitizes values to prevent XSS attacks.
</ResponseField>
