...
- When `$ref` resolves to an object, Doxa merges any sibling keys in the same block on top of the referenced content, allowing those keys to take precedence over matching keys in the reference.
- When `$ref` resolves to a non-object value such as an array, Doxa ignores any sibling keys.
...
Whether to load LaTeX stylesheets. By default, Doxa auto-detects LaTeX usage.
...
OpenAPI spec field keys to surface as post pills next to every parameter name. For each key, Doxa reads the value on the schema and renders it as a pill: strings render verbatim, `true` renders the key name, numbers stringify, and arrays render one pill per element. Doxa skips `false`, `null`, empty strings, and objects.
...
| `integrations.telemetry.enabled` | boolean | No | Enable Doxa telemetry. When `false`, feedback features are also disabled. |
