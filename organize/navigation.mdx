...
<Note>
  When you add the `openapi` property to a navigation element (such as an anchor, tab, or group) without specifying any pages, Doxa automatically generates pages for **all endpoints** defined in your OpenAPI specification.
...
Doxa uses the first version in the `versions` array as the default. Use the `default` field to specify a different version as the default.
