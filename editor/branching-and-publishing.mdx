...
**Saving** happens automatically. Doxa stores your edits on its servers and persists them across browser tabs, devices, and network interruptions.
...
<Note>
  Your live site updates after Doxa builds and deploys your changes. This typically takes 30 seconds to a few minutes. Check the deployment status on your [dashboard](https://app.doxa.com).
</Note>
...
Every time you save changes to a feature branch, Doxa builds a preview deployment—a temporary URL where your changes render exactly as they look when published.

### Access and share a preview

1. Click **Publish** in the editor toolbar.
1. In the publish menu, click the preview URL. The URL format is `organization-branch-name.doxa.app`.
    <Frame>
...
Preview URLs are publicly accessible by default. To restrict access to members of your Doxa organization, enable preview authentication in the [Add-ons](https://app.doxa.com/products/addons) page of your dashboard.

### Share editor links

To invite a teammate to a specific page on a branch, copy the URL from your browser's address bar and share it. Anyone with access to your Doxa organization can open the link directly in their editor session.

The URL format is:

```text
https://app.doxa.com/{org}/{project}/editor/{branch}/~/{filepath}
```

For example: `https://app.doxa.com/acme/docs/editor/main/~/guides/quickstart.mdx`
...
When the [Doxa agent](/agent) edits a page through the API or MCP, it appears in the editor like any other collaborator. You see the agent's avatar in the toolbar and a live cursor at its most recent edit location. The cursor clears automatically after the agent finishes.
...
Sign commits with your GitHub account by authorizing it in your [account settings](https://app.doxa.com/settings/account). Without authorization, the Doxa GitHub App signs commits made in the web editor.
...
|---|---|
| Edit a page | Changes auto-save to Doxa servers. No Git commit yet. |
