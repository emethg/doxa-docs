...
When the [Doxa agent](/agent) edits a page through the API or MCP, it appears in the web editor like any other collaborator. If you have the page open on the same branch, you see the agent's avatar and name at the top of the editor and a live cursor at the location of its most recent edit.
...
2. Click the preview URL to open the preview deployment in a new tab. The URL format is `organization-branch-name.doxa.app`.
...
Preview URLs are publicly accessible by default. Enable preview authentication in the [Add-ons](https://app.doxa.com/products/addons) page of your dashboard to restrict access to authenticated organization members.
...
The URL format is: `https://app.doxa.com/{org}/{project}/editor/{branch}/~/{filepath}`

For example, `https://app.doxa.com/acme/docs/editor/main/~/guides/quickstart.mdx` opens `quickstart.mdx` on the `main` branch.

Anyone with access to your Doxa organization can use the link to open the file directly in the editor.
